window.DREAMSTONE_DATA = {
  "generatedAt": "2026-06-09T13:43:58.474Z",
  "sourceWorkbook": "Dreamstone Mysteries _ Dex (temporary).xlsx",
  "dex": [
    {
      "number": 1,
      "name": "Gothita",
      "region": "",
      "location": "Starter",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/gothita.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        2
      ],
      "stats": {
        "hp": 45,
        "atk": 30,
        "def": 50,
        "spd": 45,
        "spa": 55,
        "spdef": 65
      },
      "bst": 290,
      "statsSource": "Pokerex"
    },
    {
      "number": 2,
      "name": "Gothorita",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/gothorita.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [
        1
      ],
      "evolvesTo": [
        3
      ],
      "stats": {
        "hp": 60,
        "atk": 45,
        "def": 70,
        "spd": 55,
        "spa": 75,
        "spdef": 85
      },
      "bst": 390,
      "statsSource": "Pokerex"
    },
    {
      "number": 3,
      "name": "Gothitelle",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/gothitelle.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [
        2
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 55,
        "def": 95,
        "spd": 65,
        "spa": 95,
        "spdef": 110
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 4,
      "name": "Timburr",
      "region": "",
      "location": "Starter",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/timburr.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        5
      ],
      "stats": {
        "hp": 75,
        "atk": 80,
        "def": 55,
        "spd": 35,
        "spa": 25,
        "spdef": 35
      },
      "bst": 305,
      "statsSource": "Pokerex"
    },
    {
      "number": 5,
      "name": "Gurdurr",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/gurdurr.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [
        4
      ],
      "evolvesTo": [
        6
      ],
      "stats": {
        "hp": 75,
        "atk": 95,
        "def": 75,
        "spd": 40,
        "spa": 40,
        "spdef": 50
      },
      "bst": 375,
      "statsSource": "Pokerex"
    },
    {
      "number": 6,
      "name": "Conkeldurr",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/conkeldurr.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [
        5
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 105,
        "atk": 140,
        "def": 95,
        "spd": 45,
        "spa": 55,
        "spdef": 65
      },
      "bst": 505,
      "statsSource": "Pokerex"
    },
    {
      "number": 7,
      "name": "Zigzagoon",
      "region": "Galar",
      "location": "Starter",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/zigzagoon-galar.png",
      "types": [
        "dark",
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        8
      ],
      "stats": {
        "hp": 38,
        "atk": 30,
        "def": 41,
        "spd": 60,
        "spa": 30,
        "spdef": 41
      },
      "bst": 240,
      "statsSource": "Pokerex"
    },
    {
      "number": 8,
      "name": "Linoone",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/linoone-galar.png",
      "types": [
        "dark",
        "normal"
      ],
      "evolvesFrom": [
        7
      ],
      "evolvesTo": [
        9
      ],
      "stats": {
        "hp": 78,
        "atk": 70,
        "def": 61,
        "spd": 100,
        "spa": 50,
        "spdef": 61
      },
      "bst": 420,
      "statsSource": "Pokerex"
    },
    {
      "number": 9,
      "name": "Obstagoon",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/obstagoon.png",
      "types": [
        "dark",
        "normal"
      ],
      "evolvesFrom": [
        8
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 93,
        "atk": 90,
        "def": 101,
        "spd": 95,
        "spa": 60,
        "spdef": 81
      },
      "bst": 520,
      "statsSource": "Pokerex"
    },
    {
      "number": 10,
      "name": "Rattata",
      "region": "Alola",
      "location": "Route 1",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/rattata-alola.png",
      "types": [
        "dark",
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        11
      ],
      "stats": {
        "hp": 30,
        "atk": 56,
        "def": 35,
        "spd": 72,
        "spa": 25,
        "spdef": 35
      },
      "bst": 253,
      "statsSource": "Pokerex"
    },
    {
      "number": 11,
      "name": "Raticate",
      "region": "Alola",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/raticate-alola.png",
      "types": [
        "dark",
        "normal"
      ],
      "evolvesFrom": [
        10
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 71,
        "def": 70,
        "spd": 77,
        "spa": 40,
        "spdef": 80
      },
      "bst": 413,
      "statsSource": "Pokerex"
    },
    {
      "number": 12,
      "name": "Pidove",
      "region": "",
      "location": "Route 1",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pidove.png",
      "types": [
        "normal",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        13
      ],
      "stats": {
        "hp": 50,
        "atk": 55,
        "def": 50,
        "spd": 43,
        "spa": 36,
        "spdef": 30
      },
      "bst": 264,
      "statsSource": "Pokerex"
    },
    {
      "number": 13,
      "name": "Tranquill",
      "region": "",
      "location": "Route 5",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/tranquill.png",
      "types": [
        "normal",
        "flying"
      ],
      "evolvesFrom": [
        12
      ],
      "evolvesTo": [
        14
      ],
      "stats": {
        "hp": 62,
        "atk": 77,
        "def": 62,
        "spd": 65,
        "spa": 50,
        "spdef": 42
      },
      "bst": 358,
      "statsSource": "Pokerex"
    },
    {
      "number": 14,
      "name": "Unfezant",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/unfezant.png",
      "types": [
        "normal",
        "flying"
      ],
      "evolvesFrom": [
        13
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 115,
        "def": 80,
        "spd": 93,
        "spa": 65,
        "spdef": 55
      },
      "bst": 488,
      "statsSource": "Pokerex"
    },
    {
      "number": 15,
      "name": "Cubone",
      "region": "",
      "location": "Route 1",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/cubone.png",
      "types": [
        "ground"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        16
      ],
      "stats": {
        "hp": 50,
        "atk": 50,
        "def": 95,
        "spd": 35,
        "spa": 40,
        "spdef": 50
      },
      "bst": 320,
      "statsSource": "Pokerex"
    },
    {
      "number": 16,
      "name": "Marowak",
      "region": "Alola",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/marowak-alola.png",
      "types": [
        "fire",
        "ghost"
      ],
      "evolvesFrom": [
        15
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 80,
        "def": 110,
        "spd": 45,
        "spa": 50,
        "spdef": 80
      },
      "bst": 425,
      "statsSource": "Pokerex"
    },
    {
      "number": 17,
      "name": "Rockruff",
      "region": "",
      "location": "Route 1",
      "rarity": "Rare",
      "notes": "Own Tempo Rockruff is in Route 2, which evolves into Dusk Form",
      "availability": "Available",
      "sprite": "assets/sprites/rockruff.png",
      "types": [
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        18
      ],
      "stats": {
        "hp": 45,
        "atk": 65,
        "def": 40,
        "spd": 60,
        "spa": 30,
        "spdef": 40
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 18,
      "name": "Lycanroc",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lycanroc.png",
      "types": [
        "rock"
      ],
      "evolvesFrom": [
        17
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 115,
        "def": 65,
        "spd": 112,
        "spa": 55,
        "spdef": 65
      },
      "bst": 487,
      "statsSource": "Pokerex"
    },
    {
      "number": 19,
      "name": "Clefairy",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/clefairy.png",
      "types": [
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        20
      ],
      "stats": {
        "hp": 70,
        "atk": 45,
        "def": 48,
        "spd": 35,
        "spa": 60,
        "spdef": 65
      },
      "bst": 323,
      "statsSource": "Pokerex"
    },
    {
      "number": 20,
      "name": "Clefable",
      "region": "",
      "location": "Victory Road",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/clefable.png",
      "types": [
        "fairy"
      ],
      "evolvesFrom": [
        19
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 70,
        "def": 73,
        "spd": 60,
        "spa": 95,
        "spdef": 90
      },
      "bst": 483,
      "statsSource": "Pokerex"
    },
    {
      "number": 21,
      "name": "Teddiursa",
      "region": "",
      "location": "Fennilahl Underpass",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/teddiursa.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        22
      ],
      "stats": {
        "hp": 60,
        "atk": 80,
        "def": 50,
        "spd": 40,
        "spa": 50,
        "spdef": 50
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 22,
      "name": "Ursaring",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/ursaring.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [
        21
      ],
      "evolvesTo": [
        23
      ],
      "stats": {
        "hp": 90,
        "atk": 130,
        "def": 75,
        "spd": 55,
        "spa": 75,
        "spdef": 75
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 23,
      "name": "Ursaluna",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Normal form; Bloodmoon isn't available",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/ursaluna.png",
      "types": [
        "ground",
        "normal"
      ],
      "evolvesFrom": [
        22
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 130,
        "atk": 140,
        "def": 105,
        "spd": 50,
        "spa": 45,
        "spdef": 80
      },
      "bst": 550,
      "statsSource": "Pokerex"
    },
    {
      "number": 24,
      "name": "Meditite",
      "region": "",
      "location": "Fennilahl Underpass",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/meditite.png",
      "types": [
        "fighting",
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        25
      ],
      "stats": {
        "hp": 30,
        "atk": 40,
        "def": 55,
        "spd": 60,
        "spa": 40,
        "spdef": 55
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 25,
      "name": "Medicham",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/medicham.png",
      "types": [
        "fighting",
        "psychic"
      ],
      "evolvesFrom": [
        24
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 60,
        "def": 75,
        "spd": 80,
        "spa": 60,
        "spdef": 75
      },
      "bst": 410,
      "statsSource": "Pokerex"
    },
    {
      "number": 26,
      "name": "Tinkatink",
      "region": "",
      "location": "Fennilahl Underpass",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-957.png",
      "types": [
        "fairy",
        "steel"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        27
      ],
      "stats": {
        "hp": 50,
        "atk": 45,
        "def": 45,
        "spd": 58,
        "spa": 35,
        "spdef": 64
      },
      "bst": 297,
      "statsSource": "Pokerex"
    },
    {
      "number": 27,
      "name": "Tinkatuff",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-958.png",
      "types": [
        "fairy",
        "steel"
      ],
      "evolvesFrom": [
        26
      ],
      "evolvesTo": [
        28
      ],
      "stats": {
        "hp": 65,
        "atk": 55,
        "def": 55,
        "spd": 78,
        "spa": 45,
        "spdef": 82
      },
      "bst": 380,
      "statsSource": "Pokerex"
    },
    {
      "number": 28,
      "name": "Tinkaton",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-959.png",
      "types": [
        "fairy",
        "steel"
      ],
      "evolvesFrom": [
        27
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 75,
        "def": 77,
        "spd": 94,
        "spa": 70,
        "spdef": 105
      },
      "bst": 506,
      "statsSource": "Pokerex"
    },
    {
      "number": 29,
      "name": "Noibat",
      "region": "",
      "location": "Fennilahl Underpass",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/noibat.png",
      "types": [
        "flying",
        "dragon"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        30
      ],
      "stats": {
        "hp": 40,
        "atk": 30,
        "def": 35,
        "spd": 55,
        "spa": 45,
        "spdef": 40
      },
      "bst": 245,
      "statsSource": "Pokerex"
    },
    {
      "number": 30,
      "name": "Noivern",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/noivern.png",
      "types": [
        "flying",
        "dragon"
      ],
      "evolvesFrom": [
        29
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 70,
        "def": 80,
        "spd": 123,
        "spa": 97,
        "spdef": 80
      },
      "bst": 535,
      "statsSource": "Pokerex"
    },
    {
      "number": 31,
      "name": "Aron",
      "region": "",
      "location": "Route 3 Caverns",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/aron.png",
      "types": [
        "steel",
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        32
      ],
      "stats": {
        "hp": 50,
        "atk": 70,
        "def": 100,
        "spd": 30,
        "spa": 40,
        "spdef": 40
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 32,
      "name": "Lairon",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lairon.png",
      "types": [
        "steel",
        "rock"
      ],
      "evolvesFrom": [
        31
      ],
      "evolvesTo": [
        33
      ],
      "stats": {
        "hp": 60,
        "atk": 90,
        "def": 140,
        "spd": 40,
        "spa": 50,
        "spdef": 50
      },
      "bst": 430,
      "statsSource": "Pokerex"
    },
    {
      "number": 33,
      "name": "Aggron",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/aggron.png",
      "types": [
        "steel",
        "rock"
      ],
      "evolvesFrom": [
        32
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 110,
        "def": 180,
        "spd": 50,
        "spa": 60,
        "spdef": 60
      },
      "bst": 530,
      "statsSource": "Pokerex"
    },
    {
      "number": 34,
      "name": "Lillipup",
      "region": "",
      "location": "Route 3",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/lillipup.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        35
      ],
      "stats": {
        "hp": 45,
        "atk": 60,
        "def": 45,
        "spd": 55,
        "spa": 25,
        "spdef": 45
      },
      "bst": 275,
      "statsSource": "Pokerex"
    },
    {
      "number": 35,
      "name": "Herdier",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/herdier.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [
        34
      ],
      "evolvesTo": [
        36
      ],
      "stats": {
        "hp": 65,
        "atk": 80,
        "def": 65,
        "spd": 60,
        "spa": 35,
        "spdef": 65
      },
      "bst": 370,
      "statsSource": "Pokerex"
    },
    {
      "number": 36,
      "name": "Stoutland",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/stoutland.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [
        35
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 110,
        "def": 90,
        "spd": 80,
        "spa": 45,
        "spdef": 90
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 37,
      "name": "Pineco",
      "region": "",
      "location": "Route 3",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pineco.png",
      "types": [
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        38
      ],
      "stats": {
        "hp": 50,
        "atk": 65,
        "def": 90,
        "spd": 15,
        "spa": 35,
        "spdef": 35
      },
      "bst": 290,
      "statsSource": "Pokerex"
    },
    {
      "number": 38,
      "name": "Forretress",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/forretress.png",
      "types": [
        "bug",
        "steel"
      ],
      "evolvesFrom": [
        37
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 90,
        "def": 140,
        "spd": 40,
        "spa": 60,
        "spdef": 60
      },
      "bst": 465,
      "statsSource": "Pokerex"
    },
    {
      "number": 39,
      "name": "Swablu",
      "region": "",
      "location": "Route 3",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/swablu.png",
      "types": [
        "normal",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        40
      ],
      "stats": {
        "hp": 45,
        "atk": 40,
        "def": 60,
        "spd": 50,
        "spa": 40,
        "spdef": 75
      },
      "bst": 310,
      "statsSource": "Pokerex"
    },
    {
      "number": 40,
      "name": "Altaria",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/altaria.png",
      "types": [
        "dragon",
        "flying"
      ],
      "evolvesFrom": [
        39
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 70,
        "def": 90,
        "spd": 80,
        "spa": 70,
        "spdef": 105
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 41,
      "name": "Poltchageist",
      "region": "",
      "location": "Gastree City",
      "rarity": "Unique",
      "notes": "Complete the teahouse quest",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-1012.png",
      "types": [
        "grass",
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        42
      ],
      "stats": {
        "hp": 40,
        "atk": 45,
        "def": 45,
        "spd": 50,
        "spa": 74,
        "spdef": 54
      },
      "bst": 308,
      "statsSource": "Pokerex"
    },
    {
      "number": 42,
      "name": "Sinistcha",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-1013.png",
      "types": [
        "grass",
        "ghost"
      ],
      "evolvesFrom": [
        41
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 71,
        "atk": 60,
        "def": 106,
        "spd": 70,
        "spa": 121,
        "spdef": 80
      },
      "bst": 508,
      "statsSource": "Pokerex"
    },
    {
      "number": 43,
      "name": "Sinistea",
      "region": "",
      "location": "Gastree City",
      "rarity": "Unique",
      "notes": "Complete the teahouse quest",
      "availability": "Available",
      "sprite": "assets/sprites/sinistea.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        44
      ],
      "stats": {
        "hp": 40,
        "atk": 45,
        "def": 45,
        "spd": 50,
        "spa": 74,
        "spdef": 54
      },
      "bst": 308,
      "statsSource": "Pokerex"
    },
    {
      "number": 44,
      "name": "Polteageist",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/polteageist.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [
        43
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 65,
        "def": 65,
        "spd": 70,
        "spa": 134,
        "spdef": 114
      },
      "bst": 508,
      "statsSource": "Pokerex"
    },
    {
      "number": 45,
      "name": "Buneary",
      "region": "",
      "location": "Route 4",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/buneary.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        46
      ],
      "stats": {
        "hp": 55,
        "atk": 66,
        "def": 44,
        "spd": 85,
        "spa": 44,
        "spdef": 56
      },
      "bst": 350,
      "statsSource": "Pokerex"
    },
    {
      "number": 46,
      "name": "Lopunny",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lopunny.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [
        45
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 65,
        "atk": 76,
        "def": 84,
        "spd": 105,
        "spa": 54,
        "spdef": 96
      },
      "bst": 480,
      "statsSource": "Pokerex"
    },
    {
      "number": 47,
      "name": "Blipbug",
      "region": "",
      "location": "Route 4",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/blipbug.png",
      "types": [
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        48
      ],
      "stats": {
        "hp": 25,
        "atk": 20,
        "def": 20,
        "spd": 45,
        "spa": 25,
        "spdef": 45
      },
      "bst": 180,
      "statsSource": "Pokerex"
    },
    {
      "number": 48,
      "name": "Dottler",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/dottler.png",
      "types": [
        "bug",
        "psychic"
      ],
      "evolvesFrom": [
        47
      ],
      "evolvesTo": [
        49
      ],
      "stats": {
        "hp": 50,
        "atk": 35,
        "def": 80,
        "spd": 30,
        "spa": 50,
        "spdef": 90
      },
      "bst": 335,
      "statsSource": "Pokerex"
    },
    {
      "number": 49,
      "name": "Orbeetle",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/orbeetle.png",
      "types": [
        "bug",
        "psychic"
      ],
      "evolvesFrom": [
        48
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 45,
        "def": 110,
        "spd": 90,
        "spa": 80,
        "spdef": 120
      },
      "bst": 505,
      "statsSource": "Pokerex"
    },
    {
      "number": 50,
      "name": "Dewpider",
      "region": "",
      "location": "Route 4",
      "rarity": "Unique",
      "notes": "Save the kids from the Dewpider attack",
      "availability": "Available",
      "sprite": "assets/sprites/dewpider.png",
      "types": [
        "water",
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        51
      ],
      "stats": {
        "hp": 38,
        "atk": 40,
        "def": 52,
        "spd": 27,
        "spa": 40,
        "spdef": 72
      },
      "bst": 269,
      "statsSource": "Pokerex"
    },
    {
      "number": 51,
      "name": "Araquanid",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/araquanid.png",
      "types": [
        "water",
        "bug"
      ],
      "evolvesFrom": [
        50
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 68,
        "atk": 70,
        "def": 92,
        "spd": 42,
        "spa": 50,
        "spdef": 132
      },
      "bst": 454,
      "statsSource": "Pokerex"
    },
    {
      "number": 52,
      "name": "Petilil",
      "region": "",
      "location": "Route 4",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/petilil.png",
      "types": [
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        53
      ],
      "stats": {
        "hp": 45,
        "atk": 35,
        "def": 50,
        "spd": 30,
        "spa": 70,
        "spdef": 50
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 53,
      "name": "Lilligant",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lilligant-hisui.png",
      "types": [
        "grass",
        "fighting"
      ],
      "evolvesFrom": [
        52
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 105,
        "def": 75,
        "spd": 105,
        "spa": 50,
        "spdef": 75
      },
      "bst": 480,
      "statsSource": "Pokerex"
    },
    {
      "number": 54,
      "name": "Chikorita",
      "region": "",
      "location": "Route 4",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/chikorita.png",
      "types": [
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        55
      ],
      "stats": {
        "hp": 45,
        "atk": 49,
        "def": 65,
        "spd": 45,
        "spa": 49,
        "spdef": 65
      },
      "bst": 318,
      "statsSource": "Pokerex"
    },
    {
      "number": 55,
      "name": "Bayleef",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/bayleef.png",
      "types": [
        "grass"
      ],
      "evolvesFrom": [
        54
      ],
      "evolvesTo": [
        56
      ],
      "stats": {
        "hp": 60,
        "atk": 62,
        "def": 80,
        "spd": 60,
        "spa": 63,
        "spdef": 80
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 56,
      "name": "Meganium",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/meganium.png",
      "types": [
        "grass"
      ],
      "evolvesFrom": [
        55
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 82,
        "def": 100,
        "spd": 80,
        "spa": 83,
        "spdef": 100
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 57,
      "name": "Chinchou",
      "region": "",
      "location": "Route 4 (Trolling Rod)",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/chinchou.png",
      "types": [
        "water",
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        58
      ],
      "stats": {
        "hp": 75,
        "atk": 38,
        "def": 38,
        "spd": 67,
        "spa": 56,
        "spdef": 56
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 58,
      "name": "Lanturn",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lanturn.png",
      "types": [
        "water",
        "electric"
      ],
      "evolvesFrom": [
        57
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 125,
        "atk": 58,
        "def": 58,
        "spd": 67,
        "spa": 76,
        "spdef": 76
      },
      "bst": 460,
      "statsSource": "Pokerex"
    },
    {
      "number": 59,
      "name": "Mankey",
      "region": "",
      "location": "Mt. Ceram",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/mankey.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        60
      ],
      "stats": {
        "hp": 40,
        "atk": 80,
        "def": 35,
        "spd": 70,
        "spa": 35,
        "spdef": 45
      },
      "bst": 305,
      "statsSource": "Pokerex"
    },
    {
      "number": 60,
      "name": "Primeape",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/primeape.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [
        59
      ],
      "evolvesTo": [
        61
      ],
      "stats": {
        "hp": 65,
        "atk": 105,
        "def": 60,
        "spd": 95,
        "spa": 60,
        "spdef": 70
      },
      "bst": 455,
      "statsSource": "Pokerex"
    },
    {
      "number": 61,
      "name": "Annihilape",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-979.png",
      "types": [
        "fighting",
        "ghost"
      ],
      "evolvesFrom": [
        60
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 110,
        "atk": 115,
        "def": 80,
        "spd": 90,
        "spa": 50,
        "spdef": 90
      },
      "bst": 535,
      "statsSource": "Pokerex"
    },
    {
      "number": 62,
      "name": "Venipede",
      "region": "",
      "location": "Mt. Ceram",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/venipede.png",
      "types": [
        "bug",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        63
      ],
      "stats": {
        "hp": 30,
        "atk": 45,
        "def": 59,
        "spd": 57,
        "spa": 30,
        "spdef": 39
      },
      "bst": 260,
      "statsSource": "Pokerex"
    },
    {
      "number": 63,
      "name": "Whirlipede",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/whirlipede.png",
      "types": [
        "bug",
        "poison"
      ],
      "evolvesFrom": [
        62
      ],
      "evolvesTo": [
        64
      ],
      "stats": {
        "hp": 40,
        "atk": 55,
        "def": 99,
        "spd": 47,
        "spa": 40,
        "spdef": 79
      },
      "bst": 360,
      "statsSource": "Pokerex"
    },
    {
      "number": 64,
      "name": "Scolipede",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/scolipede.png",
      "types": [
        "bug",
        "poison"
      ],
      "evolvesFrom": [
        63
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 100,
        "def": 89,
        "spd": 112,
        "spa": 55,
        "spdef": 69
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 65,
      "name": "Growlithe",
      "region": "Hisui",
      "location": "Mt. Ceram",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/growlithe-hisui.png",
      "types": [
        "fire",
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        66
      ],
      "stats": {
        "hp": 60,
        "atk": 75,
        "def": 45,
        "spd": 55,
        "spa": 65,
        "spdef": 50
      },
      "bst": 350,
      "statsSource": "Pokerex"
    },
    {
      "number": 66,
      "name": "Arcanine",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/arcanine-hisui.png",
      "types": [
        "fire",
        "rock"
      ],
      "evolvesFrom": [
        65
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 115,
        "def": 80,
        "spd": 90,
        "spa": 95,
        "spdef": 80
      },
      "bst": 555,
      "statsSource": "Pokerex"
    },
    {
      "number": 67,
      "name": "Sizzlipede",
      "region": "",
      "location": "Mt. Ceram (Caves)",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/sizzlipede.png",
      "types": [
        "fire",
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        68
      ],
      "stats": {
        "hp": 50,
        "atk": 65,
        "def": 45,
        "spd": 45,
        "spa": 50,
        "spdef": 50
      },
      "bst": 305,
      "statsSource": "Pokerex"
    },
    {
      "number": 68,
      "name": "Centiskorch",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/centiskorch.png",
      "types": [
        "fire",
        "bug"
      ],
      "evolvesFrom": [
        67
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 100,
        "atk": 115,
        "def": 65,
        "spd": 65,
        "spa": 90,
        "spdef": 90
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 69,
      "name": "Rolycoly",
      "region": "",
      "location": "Mt. Ceram (Caves)",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/rolycoly.png",
      "types": [
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        70
      ],
      "stats": {
        "hp": 30,
        "atk": 40,
        "def": 50,
        "spd": 30,
        "spa": 40,
        "spdef": 50
      },
      "bst": 240,
      "statsSource": "Pokerex"
    },
    {
      "number": 70,
      "name": "Carkol",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/carkol.png",
      "types": [
        "rock",
        "fire"
      ],
      "evolvesFrom": [
        69
      ],
      "evolvesTo": [
        71
      ],
      "stats": {
        "hp": 80,
        "atk": 60,
        "def": 90,
        "spd": 50,
        "spa": 60,
        "spdef": 70
      },
      "bst": 410,
      "statsSource": "Pokerex"
    },
    {
      "number": 71,
      "name": "Coalossal",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/coalossal.png",
      "types": [
        "rock",
        "fire"
      ],
      "evolvesFrom": [
        70
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 110,
        "atk": 80,
        "def": 120,
        "spd": 30,
        "spa": 80,
        "spdef": 90
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 72,
      "name": "Cyndaquil",
      "region": "",
      "location": "Mt. Ceram (Caves)",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/cyndaquil.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        73
      ],
      "stats": {
        "hp": 39,
        "atk": 52,
        "def": 43,
        "spd": 65,
        "spa": 60,
        "spdef": 50
      },
      "bst": 309,
      "statsSource": "Pokerex"
    },
    {
      "number": 73,
      "name": "Quilava",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/quilava.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [
        72
      ],
      "evolvesTo": [
        74
      ],
      "stats": {
        "hp": 58,
        "atk": 64,
        "def": 58,
        "spd": 80,
        "spa": 80,
        "spdef": 65
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 74,
      "name": "Typhlosion",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/typhlosion-hisui.png",
      "types": [
        "fire",
        "ghost"
      ],
      "evolvesFrom": [
        73
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 73,
        "atk": 84,
        "def": 78,
        "spd": 95,
        "spa": 119,
        "spdef": 85
      },
      "bst": 534,
      "statsSource": "Pokerex"
    },
    {
      "number": 75,
      "name": "Nickit",
      "region": "",
      "location": "Route 5",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/nickit.png",
      "types": [
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        76
      ],
      "stats": {
        "hp": 40,
        "atk": 28,
        "def": 28,
        "spd": 50,
        "spa": 47,
        "spdef": 52
      },
      "bst": 245,
      "statsSource": "Pokerex"
    },
    {
      "number": 76,
      "name": "Thievul",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/thievul.png",
      "types": [
        "dark"
      ],
      "evolvesFrom": [
        75
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 58,
        "def": 58,
        "spd": 90,
        "spa": 87,
        "spdef": 92
      },
      "bst": 455,
      "statsSource": "Pokerex"
    },
    {
      "number": 77,
      "name": "Murkrow",
      "region": "",
      "location": "Route 5",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/murkrow.png",
      "types": [
        "dark",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        78
      ],
      "stats": {
        "hp": 60,
        "atk": 85,
        "def": 42,
        "spd": 91,
        "spa": 85,
        "spdef": 42
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 78,
      "name": "Honchkrow",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/honchkrow.png",
      "types": [
        "dark",
        "flying"
      ],
      "evolvesFrom": [
        77
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 100,
        "atk": 125,
        "def": 52,
        "spd": 71,
        "spa": 105,
        "spdef": 52
      },
      "bst": 505,
      "statsSource": "Pokerex"
    },
    {
      "number": 79,
      "name": "Nymble",
      "region": "",
      "location": "Route 5",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-919.png",
      "types": [
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        80
      ],
      "stats": {
        "hp": 33,
        "atk": 46,
        "def": 40,
        "spd": 45,
        "spa": 21,
        "spdef": 25
      },
      "bst": 210,
      "statsSource": "Pokerex"
    },
    {
      "number": 80,
      "name": "Lokix",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-920.png",
      "types": [
        "bug",
        "dark"
      ],
      "evolvesFrom": [
        79
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 71,
        "atk": 102,
        "def": 78,
        "spd": 92,
        "spa": 52,
        "spdef": 55
      },
      "bst": 450,
      "statsSource": "Pokerex"
    },
    {
      "number": 81,
      "name": "Drowzee",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/drowzee.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        82
      ],
      "stats": {
        "hp": 60,
        "atk": 48,
        "def": 45,
        "spd": 42,
        "spa": 43,
        "spdef": 90
      },
      "bst": 328,
      "statsSource": "Pokerex"
    },
    {
      "number": 82,
      "name": "Hypno",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/hypno.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [
        81
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 73,
        "def": 70,
        "spd": 67,
        "spa": 73,
        "spdef": 115
      },
      "bst": 483,
      "statsSource": "Pokerex"
    },
    {
      "number": 83,
      "name": "Morelull",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/morelull.png",
      "types": [
        "grass",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        86
      ],
      "stats": {
        "hp": 40,
        "atk": 35,
        "def": 55,
        "spd": 15,
        "spa": 65,
        "spdef": 75
      },
      "bst": 285,
      "statsSource": "Pokerex"
    },
    {
      "number": 86,
      "name": "Shiinotic",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/shiinotic.png",
      "types": [
        "grass",
        "fairy"
      ],
      "evolvesFrom": [
        83
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 45,
        "def": 80,
        "spd": 30,
        "spa": 90,
        "spdef": 100
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 84,
      "name": "Foongus",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/foongus.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        85
      ],
      "stats": {
        "hp": 69,
        "atk": 55,
        "def": 45,
        "spd": 15,
        "spa": 55,
        "spdef": 55
      },
      "bst": 294,
      "statsSource": "Pokerex"
    },
    {
      "number": 85,
      "name": "Amoonguss",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/amoonguss.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [
        84
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 114,
        "atk": 85,
        "def": 70,
        "spd": 30,
        "spa": 85,
        "spdef": 80
      },
      "bst": 464,
      "statsSource": "Pokerex"
    },
    {
      "number": 87,
      "name": "Impidimp",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/impidimp.png",
      "types": [
        "dark",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        88
      ],
      "stats": {
        "hp": 45,
        "atk": 45,
        "def": 30,
        "spd": 50,
        "spa": 55,
        "spdef": 40
      },
      "bst": 265,
      "statsSource": "Pokerex"
    },
    {
      "number": 88,
      "name": "Morgrem",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/morgrem.png",
      "types": [
        "dark",
        "fairy"
      ],
      "evolvesFrom": [
        87
      ],
      "evolvesTo": [
        89
      ],
      "stats": {
        "hp": 65,
        "atk": 60,
        "def": 45,
        "spd": 70,
        "spa": 75,
        "spdef": 55
      },
      "bst": 370,
      "statsSource": "Pokerex"
    },
    {
      "number": 89,
      "name": "Grimmsnarl",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/grimmsnarl.png",
      "types": [
        "dark",
        "fairy"
      ],
      "evolvesFrom": [
        88
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 120,
        "def": 65,
        "spd": 60,
        "spa": 95,
        "spdef": 75
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 90,
      "name": "Fennekin",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/fennekin.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        91
      ],
      "stats": {
        "hp": 40,
        "atk": 45,
        "def": 40,
        "spd": 60,
        "spa": 62,
        "spdef": 60
      },
      "bst": 307,
      "statsSource": "Pokerex"
    },
    {
      "number": 91,
      "name": "Braixen",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/braixen.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [
        90
      ],
      "evolvesTo": [
        92
      ],
      "stats": {
        "hp": 59,
        "atk": 59,
        "def": 58,
        "spd": 73,
        "spa": 90,
        "spdef": 70
      },
      "bst": 409,
      "statsSource": "Pokerex"
    },
    {
      "number": 92,
      "name": "Delphox",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/delphox.png",
      "types": [
        "fire",
        "psychic"
      ],
      "evolvesFrom": [
        91
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 69,
        "def": 72,
        "spd": 104,
        "spa": 114,
        "spdef": 100
      },
      "bst": 534,
      "statsSource": "Pokerex"
    },
    {
      "number": 93,
      "name": "Zorua",
      "region": "Hisui",
      "location": "Ivy Forest",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/zorua-hisui.png",
      "types": [
        "normal",
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        94
      ],
      "stats": {
        "hp": 35,
        "atk": 60,
        "def": 40,
        "spd": 70,
        "spa": 85,
        "spdef": 40
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 94,
      "name": "Zoroark",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/zoroark-hisui.png",
      "types": [
        "normal",
        "ghost"
      ],
      "evolvesFrom": [
        93
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 55,
        "atk": 100,
        "def": 60,
        "spd": 110,
        "spa": 125,
        "spdef": 60
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 95,
      "name": "Budew",
      "region": "",
      "location": "Fennilahl Town",
      "rarity": "Unique",
      "notes": "Give the Budew 3 berries - Oran, Pecha, Rawst",
      "availability": "Available",
      "sprite": "assets/sprites/budew.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        96
      ],
      "stats": {
        "hp": 40,
        "atk": 30,
        "def": 35,
        "spd": 55,
        "spa": 50,
        "spdef": 70
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 96,
      "name": "Roselia",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/roselia.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [
        95
      ],
      "evolvesTo": [
        97
      ],
      "stats": {
        "hp": 50,
        "atk": 60,
        "def": 45,
        "spd": 65,
        "spa": 100,
        "spdef": 80
      },
      "bst": 400,
      "statsSource": "Pokerex"
    },
    {
      "number": 97,
      "name": "Roserade",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/roserade.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [
        96
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 70,
        "def": 65,
        "spd": 90,
        "spa": 125,
        "spdef": 105
      },
      "bst": 515,
      "statsSource": "Pokerex"
    },
    {
      "number": 98,
      "name": "Drilbur",
      "region": "",
      "location": "Ceram Base Camp",
      "rarity": "Unique",
      "notes": "Click on the big drill near the Supply Depot",
      "availability": "Available",
      "sprite": "assets/sprites/drilbur.png",
      "types": [
        "ground"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        99
      ],
      "stats": {
        "hp": 60,
        "atk": 85,
        "def": 40,
        "spd": 68,
        "spa": 30,
        "spdef": 45
      },
      "bst": 328,
      "statsSource": "Pokerex"
    },
    {
      "number": 99,
      "name": "Excadrill",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/excadrill.png",
      "types": [
        "ground",
        "steel"
      ],
      "evolvesFrom": [
        98
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 110,
        "atk": 135,
        "def": 60,
        "spd": 88,
        "spa": 50,
        "spdef": 65
      },
      "bst": 508,
      "statsSource": "Pokerex"
    },
    {
      "number": 100,
      "name": "Skrelp",
      "region": "",
      "location": "Silversun Sewers",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/skrelp.png",
      "types": [
        "poison",
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        101
      ],
      "stats": {
        "hp": 50,
        "atk": 60,
        "def": 60,
        "spd": 30,
        "spa": 60,
        "spdef": 60
      },
      "bst": 320,
      "statsSource": "Pokerex"
    },
    {
      "number": 101,
      "name": "Dragalge",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/dragalge.png",
      "types": [
        "poison",
        "dragon"
      ],
      "evolvesFrom": [
        100
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 65,
        "atk": 75,
        "def": 90,
        "spd": 44,
        "spa": 97,
        "spdef": 123
      },
      "bst": 494,
      "statsSource": "Pokerex"
    },
    {
      "number": 102,
      "name": "Grimer",
      "region": "Alola",
      "location": "Silversun Sewers",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/grimer-alola.png",
      "types": [
        "poison",
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        103
      ],
      "stats": {
        "hp": 80,
        "atk": 80,
        "def": 50,
        "spd": 25,
        "spa": 40,
        "spdef": 50
      },
      "bst": 325,
      "statsSource": "Pokerex"
    },
    {
      "number": 103,
      "name": "Muk",
      "region": "Alola",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/muk-alola.png",
      "types": [
        "poison",
        "dark"
      ],
      "evolvesFrom": [
        102
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 105,
        "atk": 105,
        "def": 75,
        "spd": 50,
        "spa": 65,
        "spdef": 100
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 104,
      "name": "Qwilfish",
      "region": "Hisui",
      "location": "Silversun Sewers",
      "rarity": "Common",
      "notes": "Also fishable at Pelluca City (Trolling Rod)",
      "availability": "Available",
      "sprite": "assets/sprites/qwilfish-hisui.png",
      "types": [
        "dark",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        105
      ],
      "stats": {
        "hp": 65,
        "atk": 95,
        "def": 85,
        "spd": 85,
        "spa": 55,
        "spdef": 55
      },
      "bst": 440,
      "statsSource": "Pokerex"
    },
    {
      "number": 105,
      "name": "Overqwil",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/overqwil.png",
      "types": [
        "dark",
        "poison"
      ],
      "evolvesFrom": [
        104
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 115,
        "def": 95,
        "spd": 85,
        "spa": 65,
        "spdef": 65
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 106,
      "name": "Toxel",
      "region": "",
      "location": "Silversun Sewers",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/toxel.png",
      "types": [
        "electric",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        107
      ],
      "stats": {
        "hp": 40,
        "atk": 38,
        "def": 35,
        "spd": 40,
        "spa": 54,
        "spdef": 35
      },
      "bst": 242,
      "statsSource": "Pokerex"
    },
    {
      "number": 107,
      "name": "Toxtricity",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/toxtricity.png",
      "types": [
        "electric",
        "poison"
      ],
      "evolvesFrom": [
        106
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 98,
        "def": 70,
        "spd": 75,
        "spa": 114,
        "spdef": 70
      },
      "bst": 502,
      "statsSource": "Pokerex"
    },
    {
      "number": 108,
      "name": "Tympole",
      "region": "",
      "location": "Route 6",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/tympole.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        109
      ],
      "stats": {
        "hp": 50,
        "atk": 50,
        "def": 40,
        "spd": 64,
        "spa": 50,
        "spdef": 40
      },
      "bst": 294,
      "statsSource": "Pokerex"
    },
    {
      "number": 109,
      "name": "Palpitoad",
      "region": "",
      "location": "Route 6",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/palpitoad.png",
      "types": [
        "water",
        "ground"
      ],
      "evolvesFrom": [
        108
      ],
      "evolvesTo": [
        110
      ],
      "stats": {
        "hp": 75,
        "atk": 65,
        "def": 55,
        "spd": 69,
        "spa": 65,
        "spdef": 55
      },
      "bst": 384,
      "statsSource": "Pokerex"
    },
    {
      "number": 110,
      "name": "Seismitoad",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/seismitoad.png",
      "types": [
        "water",
        "ground"
      ],
      "evolvesFrom": [
        109
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 105,
        "atk": 95,
        "def": 75,
        "spd": 74,
        "spa": 85,
        "spdef": 75
      },
      "bst": 509,
      "statsSource": "Pokerex"
    },
    {
      "number": 111,
      "name": "Yanma",
      "region": "",
      "location": "Route 6",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/yanma.png",
      "types": [
        "bug",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        112
      ],
      "stats": {
        "hp": 65,
        "atk": 65,
        "def": 45,
        "spd": 95,
        "spa": 75,
        "spdef": 45
      },
      "bst": 390,
      "statsSource": "Pokerex"
    },
    {
      "number": 112,
      "name": "Yanmega",
      "region": "",
      "location": "Route 6",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/yanmega.png",
      "types": [
        "bug",
        "flying"
      ],
      "evolvesFrom": [
        111
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 86,
        "atk": 76,
        "def": 86,
        "spd": 95,
        "spa": 116,
        "spdef": 56
      },
      "bst": 515,
      "statsSource": "Pokerex"
    },
    {
      "number": 113,
      "name": "Croagunk",
      "region": "",
      "location": "Route 6",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/croagunk.png",
      "types": [
        "poison",
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        114
      ],
      "stats": {
        "hp": 48,
        "atk": 61,
        "def": 40,
        "spd": 50,
        "spa": 61,
        "spdef": 40
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 114,
      "name": "Toxicroak",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/toxicroak.png",
      "types": [
        "poison",
        "fighting"
      ],
      "evolvesFrom": [
        113
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 83,
        "atk": 106,
        "def": 65,
        "spd": 85,
        "spa": 86,
        "spdef": 65
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 115,
      "name": "Wooper",
      "region": "Paldea",
      "location": "Route 6",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/wooper.png",
      "types": [
        "poison",
        "ground"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        116
      ],
      "stats": {
        "hp": 55,
        "atk": 45,
        "def": 45,
        "spd": 15,
        "spa": 25,
        "spdef": 25
      },
      "bst": 210,
      "statsSource": "Pokerex"
    },
    {
      "number": 116,
      "name": "Clodsire",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-980.png",
      "types": [
        "poison",
        "ground"
      ],
      "evolvesFrom": [
        115
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 130,
        "atk": 75,
        "def": 60,
        "spd": 20,
        "spa": 45,
        "spdef": 100
      },
      "bst": 430,
      "statsSource": "Pokerex"
    },
    {
      "number": 117,
      "name": "Totodile",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/totodile.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        118
      ],
      "stats": {
        "hp": 50,
        "atk": 65,
        "def": 64,
        "spd": 43,
        "spa": 44,
        "spdef": 48
      },
      "bst": 314,
      "statsSource": "Pokerex"
    },
    {
      "number": 118,
      "name": "Croconaw",
      "region": "",
      "location": "Route 6",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/croconaw.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        117
      ],
      "evolvesTo": [
        119
      ],
      "stats": {
        "hp": 65,
        "atk": 80,
        "def": 80,
        "spd": 58,
        "spa": 59,
        "spdef": 63
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 119,
      "name": "Feraligatr",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/feraligatr.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        118
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 105,
        "def": 100,
        "spd": 78,
        "spa": 79,
        "spdef": 83
      },
      "bst": 530,
      "statsSource": "Pokerex"
    },
    {
      "number": 120,
      "name": "Toedscool",
      "region": "",
      "location": "Route 6",
      "rarity": "Common",
      "notes": "Fishing - Trolling Rod",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-948.png",
      "types": [
        "ground",
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        121
      ],
      "stats": {
        "hp": 40,
        "atk": 40,
        "def": 35,
        "spd": 70,
        "spa": 50,
        "spdef": 100
      },
      "bst": 335,
      "statsSource": "Pokerex"
    },
    {
      "number": 121,
      "name": "Toedscruel",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-949.png",
      "types": [
        "ground",
        "grass"
      ],
      "evolvesFrom": [
        120
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 70,
        "def": 65,
        "spd": 100,
        "spa": 80,
        "spdef": 120
      },
      "bst": 515,
      "statsSource": "Pokerex"
    },
    {
      "number": 122,
      "name": "Feebas",
      "region": "",
      "location": "Route 5",
      "rarity": "Unique",
      "notes": "Offered by Greta the traveling merchant (Ugly Fish)",
      "availability": "Available",
      "sprite": "assets/sprites/feebas.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        123
      ],
      "stats": {
        "hp": 20,
        "atk": 15,
        "def": 20,
        "spd": 80,
        "spa": 10,
        "spdef": 55
      },
      "bst": 200,
      "statsSource": "Pokerex"
    },
    {
      "number": 123,
      "name": "Milotic",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/milotic.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        122
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 60,
        "def": 79,
        "spd": 81,
        "spa": 100,
        "spdef": 125
      },
      "bst": 540,
      "statsSource": "Pokerex"
    },
    {
      "number": 124,
      "name": "Basculin",
      "region": "",
      "location": "Route 6",
      "rarity": "Uncommon",
      "notes": "Fishing - Casting Rod",
      "availability": "Available",
      "sprite": "assets/sprites/basculin.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        125
      ],
      "stats": {
        "hp": 70,
        "atk": 92,
        "def": 65,
        "spd": 98,
        "spa": 80,
        "spdef": 55
      },
      "bst": 460,
      "statsSource": "Pokerex"
    },
    {
      "number": 125,
      "name": "Basculegion",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/basculegion.png",
      "types": [
        "water",
        "ghost"
      ],
      "evolvesFrom": [
        124
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 120,
        "atk": 112,
        "def": 65,
        "spd": 78,
        "spa": 80,
        "spdef": 75
      },
      "bst": 530,
      "statsSource": "Pokerex"
    },
    {
      "number": 126,
      "name": "Fletchling",
      "region": "",
      "location": "Mt Ceram",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/fletchling.png",
      "types": [
        "normal",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        127
      ],
      "stats": {
        "hp": 45,
        "atk": 50,
        "def": 43,
        "spd": 62,
        "spa": 40,
        "spdef": 38
      },
      "bst": 278,
      "statsSource": "Pokerex"
    },
    {
      "number": 127,
      "name": "Fletchinder",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/fletchinder.png",
      "types": [
        "fire",
        "flying"
      ],
      "evolvesFrom": [
        126
      ],
      "evolvesTo": [
        128
      ],
      "stats": {
        "hp": 62,
        "atk": 73,
        "def": 55,
        "spd": 84,
        "spa": 56,
        "spdef": 52
      },
      "bst": 382,
      "statsSource": "Pokerex"
    },
    {
      "number": 128,
      "name": "Talonflame",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/talonflame.png",
      "types": [
        "fire",
        "flying"
      ],
      "evolvesFrom": [
        127
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 78,
        "atk": 81,
        "def": 71,
        "spd": 126,
        "spa": 74,
        "spdef": 69
      },
      "bst": 499,
      "statsSource": "Pokerex"
    },
    {
      "number": 129,
      "name": "Applin",
      "region": "",
      "location": "Pelluca City",
      "rarity": "Unique",
      "notes": "Complete the chef's refrigerator quest",
      "availability": "Available",
      "sprite": "assets/sprites/applin.png",
      "types": [
        "grass",
        "dragon"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        130,
        131,
        132
      ],
      "stats": {
        "hp": 40,
        "atk": 40,
        "def": 80,
        "spd": 20,
        "spa": 40,
        "spdef": 40
      },
      "bst": 260,
      "statsSource": "Pokerex"
    },
    {
      "number": 130,
      "name": "Flapple",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/flapple.png",
      "types": [
        "grass",
        "dragon"
      ],
      "evolvesFrom": [
        129
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 110,
        "def": 80,
        "spd": 70,
        "spa": 95,
        "spdef": 60
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 131,
      "name": "Appletun",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/appletun.png",
      "types": [
        "grass",
        "dragon"
      ],
      "evolvesFrom": [
        129
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 110,
        "atk": 85,
        "def": 80,
        "spd": 30,
        "spa": 100,
        "spdef": 80
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 132,
      "name": "Dipplin",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-1011.png",
      "types": [
        "grass",
        "dragon"
      ],
      "evolvesFrom": [
        129
      ],
      "evolvesTo": [
        133
      ],
      "stats": {
        "hp": 80,
        "atk": 80,
        "def": 110,
        "spd": 40,
        "spa": 95,
        "spdef": 80
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 133,
      "name": "Hydrapple",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-1019.png",
      "types": [
        "grass",
        "dragon"
      ],
      "evolvesFrom": [
        132
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 106,
        "atk": 80,
        "def": 110,
        "spd": 44,
        "spa": 120,
        "spdef": 80
      },
      "bst": 540,
      "statsSource": "Pokerex"
    },
    {
      "number": 134,
      "name": "Shuppet",
      "region": "",
      "location": "Mirroh Base Camp Graveyard",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/shuppet.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        135
      ],
      "stats": {
        "hp": 44,
        "atk": 75,
        "def": 35,
        "spd": 45,
        "spa": 63,
        "spdef": 33
      },
      "bst": 295,
      "statsSource": "Pokerex"
    },
    {
      "number": 135,
      "name": "Banette",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/banette.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [
        134
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 64,
        "atk": 115,
        "def": 65,
        "spd": 65,
        "spa": 83,
        "spdef": 63
      },
      "bst": 455,
      "statsSource": "Pokerex"
    },
    {
      "number": 136,
      "name": "Diglett",
      "region": "",
      "location": "Mt Ceram Caves",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/diglett.png",
      "types": [
        "ground"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        137
      ],
      "stats": {
        "hp": 10,
        "atk": 55,
        "def": 25,
        "spd": 95,
        "spa": 35,
        "spdef": 45
      },
      "bst": 265,
      "statsSource": "Pokerex"
    },
    {
      "number": 137,
      "name": "Dugtrio",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/dugtrio.png",
      "types": [
        "ground"
      ],
      "evolvesFrom": [
        136
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 35,
        "atk": 100,
        "def": 50,
        "spd": 120,
        "spa": 50,
        "spdef": 70
      },
      "bst": 425,
      "statsSource": "Pokerex"
    },
    {
      "number": 138,
      "name": "Greavard",
      "region": "",
      "location": "Mirroh Base Camp Graveyard",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-971.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        139
      ],
      "stats": {
        "hp": 50,
        "atk": 61,
        "def": 60,
        "spd": 34,
        "spa": 30,
        "spdef": 55
      },
      "bst": 290,
      "statsSource": "Pokerex"
    },
    {
      "number": 139,
      "name": "Houndstone",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-972.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [
        138
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 72,
        "atk": 101,
        "def": 100,
        "spd": 68,
        "spa": 50,
        "spdef": 97
      },
      "bst": 488,
      "statsSource": "Pokerex"
    },
    {
      "number": 140,
      "name": "Chingling",
      "region": "",
      "location": "Mirroh Base Camp Graveyard",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/chingling.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        141
      ],
      "stats": {
        "hp": 45,
        "atk": 30,
        "def": 50,
        "spd": 45,
        "spa": 65,
        "spdef": 50
      },
      "bst": 285,
      "statsSource": "Pokerex"
    },
    {
      "number": 141,
      "name": "Chimecho",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/chimecho.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [
        140
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 50,
        "def": 80,
        "spd": 65,
        "spa": 95,
        "spdef": 90
      },
      "bst": 455,
      "statsSource": "Pokerex"
    },
    {
      "number": 142,
      "name": "Stantler",
      "region": "",
      "location": "Mt Mirroh",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/stantler.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        143
      ],
      "stats": {
        "hp": 73,
        "atk": 95,
        "def": 62,
        "spd": 85,
        "spa": 85,
        "spdef": 65
      },
      "bst": 465,
      "statsSource": "Pokerex"
    },
    {
      "number": 143,
      "name": "Wyrdeer",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/wyrdeer.png",
      "types": [
        "normal",
        "psychic"
      ],
      "evolvesFrom": [
        142
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 103,
        "atk": 105,
        "def": 72,
        "spd": 65,
        "spa": 105,
        "spdef": 75
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 144,
      "name": "Vulpix",
      "region": "Alola",
      "location": "Mt Mirroh",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/vulpix-alola.png",
      "types": [
        "ice"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        145
      ],
      "stats": {
        "hp": 38,
        "atk": 41,
        "def": 40,
        "spd": 65,
        "spa": 50,
        "spdef": 65
      },
      "bst": 299,
      "statsSource": "Pokerex"
    },
    {
      "number": 145,
      "name": "Ninetales",
      "region": "Alola",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/ninetales-alola.png",
      "types": [
        "ice",
        "fairy"
      ],
      "evolvesFrom": [
        144
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 73,
        "atk": 67,
        "def": 75,
        "spd": 109,
        "spa": 81,
        "spdef": 100
      },
      "bst": 505,
      "statsSource": "Pokerex"
    },
    {
      "number": 146,
      "name": "Rufflet",
      "region": "",
      "location": "Mt Mirroh",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/rufflet.png",
      "types": [
        "normal",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        147
      ],
      "stats": {
        "hp": 70,
        "atk": 83,
        "def": 50,
        "spd": 60,
        "spa": 37,
        "spdef": 50
      },
      "bst": 350,
      "statsSource": "Pokerex"
    },
    {
      "number": 147,
      "name": "Braviary",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/braviary-hisui.png",
      "types": [
        "psychic",
        "flying"
      ],
      "evolvesFrom": [
        146
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 110,
        "atk": 83,
        "def": 70,
        "spd": 65,
        "spa": 112,
        "spdef": 70
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 148,
      "name": "Scatterbug",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/scatterbug.png",
      "types": [
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        149
      ],
      "stats": {
        "hp": 38,
        "atk": 35,
        "def": 40,
        "spd": 35,
        "spa": 27,
        "spdef": 25
      },
      "bst": 200,
      "statsSource": "Pokerex"
    },
    {
      "number": 149,
      "name": "Spewpa",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/spewpa.png",
      "types": [
        "bug"
      ],
      "evolvesFrom": [
        148
      ],
      "evolvesTo": [
        150
      ],
      "stats": {
        "hp": 45,
        "atk": 22,
        "def": 60,
        "spd": 29,
        "spa": 27,
        "spdef": 30
      },
      "bst": 213,
      "statsSource": "Pokerex"
    },
    {
      "number": 150,
      "name": "Vivillon",
      "region": "",
      "location": "Mt Mirroh",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/vivillon.png",
      "types": [
        "bug",
        "flying"
      ],
      "evolvesFrom": [
        149
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 52,
        "def": 50,
        "spd": 89,
        "spa": 90,
        "spdef": 50
      },
      "bst": 411,
      "statsSource": "Pokerex"
    },
    {
      "number": 151,
      "name": "Bergmite",
      "region": "",
      "location": "Mt Mirroh Interior 1F",
      "rarity": "Unique",
      "notes": "Click on the rocks - 3 of them are Bergmites",
      "availability": "Available",
      "sprite": "assets/sprites/bergmite.png",
      "types": [
        "ice"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        152
      ],
      "stats": {
        "hp": 55,
        "atk": 69,
        "def": 85,
        "spd": 28,
        "spa": 32,
        "spdef": 35
      },
      "bst": 304,
      "statsSource": "Pokerex"
    },
    {
      "number": 152,
      "name": "Avalugg",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/avalugg-hisui.png",
      "types": [
        "ice",
        "rock"
      ],
      "evolvesFrom": [
        151
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 127,
        "def": 184,
        "spd": 38,
        "spa": 34,
        "spdef": 36
      },
      "bst": 514,
      "statsSource": "Pokerex"
    },
    {
      "number": 153,
      "name": "Sandshrew",
      "region": "Alola",
      "location": "Mt Mirroh Interior 1F",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/sandshrew-alola.png",
      "types": [
        "ice",
        "steel"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        154
      ],
      "stats": {
        "hp": 50,
        "atk": 75,
        "def": 90,
        "spd": 40,
        "spa": 10,
        "spdef": 35
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 154,
      "name": "Sandslash",
      "region": "Alola",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/sandslash-alola.png",
      "types": [
        "ice",
        "steel"
      ],
      "evolvesFrom": [
        153
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 100,
        "def": 120,
        "spd": 65,
        "spa": 25,
        "spdef": 65
      },
      "bst": 450,
      "statsSource": "Pokerex"
    },
    {
      "number": 155,
      "name": "Swinub",
      "region": "",
      "location": "Mt Mirroh Interior 1F",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/swinub.png",
      "types": [
        "ice",
        "ground"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        156
      ],
      "stats": {
        "hp": 50,
        "atk": 50,
        "def": 40,
        "spd": 50,
        "spa": 30,
        "spdef": 30
      },
      "bst": 250,
      "statsSource": "Pokerex"
    },
    {
      "number": 156,
      "name": "Piloswine",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/piloswine.png",
      "types": [
        "ice",
        "ground"
      ],
      "evolvesFrom": [
        155
      ],
      "evolvesTo": [
        157
      ],
      "stats": {
        "hp": 100,
        "atk": 100,
        "def": 80,
        "spd": 50,
        "spa": 60,
        "spdef": 60
      },
      "bst": 450,
      "statsSource": "Pokerex"
    },
    {
      "number": 157,
      "name": "Mamoswine",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/mamoswine.png",
      "types": [
        "ice",
        "ground"
      ],
      "evolvesFrom": [
        156
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 110,
        "atk": 130,
        "def": 80,
        "spd": 80,
        "spa": 70,
        "spdef": 60
      },
      "bst": 530,
      "statsSource": "Pokerex"
    },
    {
      "number": 158,
      "name": "Glimmet",
      "region": "",
      "location": "Mt Mirroh Interior 1F",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-969.png",
      "types": [
        "rock",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        159
      ],
      "stats": {
        "hp": 48,
        "atk": 35,
        "def": 42,
        "spd": 60,
        "spa": 105,
        "spdef": 60
      },
      "bst": 350,
      "statsSource": "Pokerex"
    },
    {
      "number": 159,
      "name": "Glimmora",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-970.png",
      "types": [
        "rock",
        "poison"
      ],
      "evolvesFrom": [
        158
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 83,
        "atk": 55,
        "def": 90,
        "spd": 86,
        "spa": 130,
        "spdef": 81
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 160,
      "name": "Cetoddle",
      "region": "",
      "location": "Mt Mirroh Interior B1F",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-974.png",
      "types": [
        "ice"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        161
      ],
      "stats": {
        "hp": 108,
        "atk": 68,
        "def": 45,
        "spd": 43,
        "spa": 30,
        "spdef": 40
      },
      "bst": 334,
      "statsSource": "Pokerex"
    },
    {
      "number": 161,
      "name": "Cetitan",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-975.png",
      "types": [
        "ice"
      ],
      "evolvesFrom": [
        160
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 170,
        "atk": 113,
        "def": 65,
        "spd": 73,
        "spa": 45,
        "spdef": 55
      },
      "bst": 521,
      "statsSource": "Pokerex"
    },
    {
      "number": 162,
      "name": "Minior",
      "region": "",
      "location": "Mt Mirroh Interior B1F",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/minior.png",
      "types": [
        "rock",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 60,
        "def": 100,
        "spd": 60,
        "spa": 60,
        "spdef": 100
      },
      "bst": 440,
      "statsSource": "Pokerex"
    },
    {
      "number": 163,
      "name": "Frigibax",
      "region": "",
      "location": "Mt Mirroh Interior B2F",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-996.png",
      "types": [
        "dragon",
        "ice"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        164
      ],
      "stats": {
        "hp": 65,
        "atk": 75,
        "def": 45,
        "spd": 55,
        "spa": 35,
        "spdef": 45
      },
      "bst": 320,
      "statsSource": "Pokerex"
    },
    {
      "number": 164,
      "name": "Arctibax",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-997.png",
      "types": [
        "dragon",
        "ice"
      ],
      "evolvesFrom": [
        163
      ],
      "evolvesTo": [
        165
      ],
      "stats": {
        "hp": 90,
        "atk": 95,
        "def": 66,
        "spd": 62,
        "spa": 45,
        "spdef": 65
      },
      "bst": 423,
      "statsSource": "Pokerex"
    },
    {
      "number": 165,
      "name": "Baxcalibur",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-998.png",
      "types": [
        "dragon",
        "ice"
      ],
      "evolvesFrom": [
        164
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 115,
        "atk": 145,
        "def": 92,
        "spd": 87,
        "spa": 75,
        "spdef": 86
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 166,
      "name": "Meowth",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/meowth-galar.png",
      "types": [
        "steel"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        167
      ],
      "stats": {
        "hp": 50,
        "atk": 65,
        "def": 55,
        "spd": 40,
        "spa": 40,
        "spdef": 40
      },
      "bst": 290,
      "statsSource": "Pokerex"
    },
    {
      "number": 167,
      "name": "Perrserker",
      "region": "",
      "location": "Mt Mirroh Peak",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/perrserker.png",
      "types": [
        "steel"
      ],
      "evolvesFrom": [
        166
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 110,
        "def": 100,
        "spd": 50,
        "spa": 50,
        "spdef": 60
      },
      "bst": 440,
      "statsSource": "Pokerex"
    },
    {
      "number": 168,
      "name": "Sneasel",
      "region": "Hisui",
      "location": "Mt Mirroh Peak",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/sneasel-hisui.png",
      "types": [
        "fighting",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        169
      ],
      "stats": {
        "hp": 55,
        "atk": 95,
        "def": 55,
        "spd": 115,
        "spa": 35,
        "spdef": 75
      },
      "bst": 430,
      "statsSource": "Pokerex"
    },
    {
      "number": 169,
      "name": "Sneasler",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/sneasler.png",
      "types": [
        "fighting",
        "poison"
      ],
      "evolvesFrom": [
        168
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 130,
        "def": 60,
        "spd": 120,
        "spa": 40,
        "spdef": 80
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 170,
      "name": "Mime_Jr",
      "region": "",
      "location": "Winterlily Hollow",
      "rarity": "Unique",
      "notes": "A Team Somber grunt abandoned this poor guy",
      "availability": "Available",
      "sprite": "assets/sprites/mime-jr.png",
      "types": [
        "psychic",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        171
      ],
      "stats": {
        "hp": 20,
        "atk": 25,
        "def": 45,
        "spd": 60,
        "spa": 70,
        "spdef": 90
      },
      "bst": 310,
      "statsSource": "Pokerex"
    },
    {
      "number": 171,
      "name": "Mr_Mime",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/mr-mime-galar.png",
      "types": [
        "ice",
        "psychic"
      ],
      "evolvesFrom": [
        170
      ],
      "evolvesTo": [
        172
      ],
      "stats": {
        "hp": 50,
        "atk": 65,
        "def": 65,
        "spd": 100,
        "spa": 90,
        "spdef": 90
      },
      "bst": 460,
      "statsSource": "Pokerex"
    },
    {
      "number": 172,
      "name": "Mr_Rime",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/mr-rime.png",
      "types": [
        "ice",
        "psychic"
      ],
      "evolvesFrom": [
        171
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 85,
        "def": 75,
        "spd": 70,
        "spa": 110,
        "spdef": 100
      },
      "bst": 520,
      "statsSource": "Pokerex"
    },
    {
      "number": 173,
      "name": "Togepi",
      "region": "",
      "location": "Route 3 Deeper",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/togepi.png",
      "types": [
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        174
      ],
      "stats": {
        "hp": 35,
        "atk": 20,
        "def": 65,
        "spd": 20,
        "spa": 40,
        "spdef": 65
      },
      "bst": 245,
      "statsSource": "Pokerex"
    },
    {
      "number": 174,
      "name": "Togetic",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/togetic.png",
      "types": [
        "fairy",
        "flying"
      ],
      "evolvesFrom": [
        173
      ],
      "evolvesTo": [
        175
      ],
      "stats": {
        "hp": 55,
        "atk": 40,
        "def": 85,
        "spd": 40,
        "spa": 80,
        "spdef": 105
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 175,
      "name": "Togekiss",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/togekiss.png",
      "types": [
        "fairy",
        "flying"
      ],
      "evolvesFrom": [
        174
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 50,
        "def": 95,
        "spd": 80,
        "spa": 120,
        "spdef": 115
      },
      "bst": 545,
      "statsSource": "Pokerex"
    },
    {
      "number": 176,
      "name": "Mawile",
      "region": "",
      "location": "Route 3 Caverns",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/mawile.png",
      "types": [
        "steel",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 85,
        "def": 85,
        "spd": 50,
        "spa": 55,
        "spdef": 55
      },
      "bst": 380,
      "statsSource": "Pokerex"
    },
    {
      "number": 177,
      "name": "Carbink",
      "region": "",
      "location": "Route 3 Caverns",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/carbink.png",
      "types": [
        "rock",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 50,
        "def": 150,
        "spd": 50,
        "spa": 50,
        "spdef": 150
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 178,
      "name": "Dreepy",
      "region": "",
      "location": "Lily Pond",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/dreepy.png",
      "types": [
        "dragon",
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        179
      ],
      "stats": {
        "hp": 28,
        "atk": 60,
        "def": 30,
        "spd": 82,
        "spa": 40,
        "spdef": 30
      },
      "bst": 270,
      "statsSource": "Pokerex"
    },
    {
      "number": 179,
      "name": "Drakloak",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/drakloak.png",
      "types": [
        "dragon",
        "ghost"
      ],
      "evolvesFrom": [
        178
      ],
      "evolvesTo": [
        180
      ],
      "stats": {
        "hp": 68,
        "atk": 80,
        "def": 50,
        "spd": 102,
        "spa": 60,
        "spdef": 50
      },
      "bst": 410,
      "statsSource": "Pokerex"
    },
    {
      "number": 180,
      "name": "Dragapult",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/dragapult.png",
      "types": [
        "dragon",
        "ghost"
      ],
      "evolvesFrom": [
        179
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 88,
        "atk": 120,
        "def": 75,
        "spd": 142,
        "spa": 100,
        "spdef": 75
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 181,
      "name": "Sableye",
      "region": "",
      "location": "Lily Pond",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/sableye.png",
      "types": [
        "dark",
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 75,
        "def": 75,
        "spd": 50,
        "spa": 65,
        "spdef": 65
      },
      "bst": 380,
      "statsSource": "Pokerex"
    },
    {
      "number": 182,
      "name": "Ditto",
      "region": "",
      "location": "Rivetshore Ranger Day-Care",
      "rarity": "",
      "notes": "Become a full ranger and then talk to the person in the daycare",
      "availability": "Available",
      "sprite": "assets/sprites/ditto.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 48,
        "atk": 48,
        "def": 48,
        "spd": 48,
        "spa": 48,
        "spdef": 48
      },
      "bst": 288,
      "statsSource": "Pokerex"
    },
    {
      "number": 183,
      "name": "Rotom",
      "region": "",
      "location": "Galecrest City",
      "rarity": "Rare",
      "notes": "Walk around in the garden for a bit",
      "availability": "Available",
      "sprite": "assets/sprites/rotom.png",
      "types": [
        "electric",
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 50,
        "def": 77,
        "spd": 91,
        "spa": 95,
        "spdef": 77
      },
      "bst": 440,
      "statsSource": "Pokerex"
    },
    {
      "number": 184,
      "name": "Litwick",
      "region": "",
      "location": "Galecrest City",
      "rarity": "Unique",
      "notes": "In the basement of one of the rich houses",
      "availability": "Available",
      "sprite": "assets/sprites/litwick.png",
      "types": [
        "ghost",
        "fire"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        185
      ],
      "stats": {
        "hp": 50,
        "atk": 30,
        "def": 55,
        "spd": 20,
        "spa": 65,
        "spdef": 55
      },
      "bst": 275,
      "statsSource": "Pokerex"
    },
    {
      "number": 185,
      "name": "Lampent",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lampent.png",
      "types": [
        "ghost",
        "fire"
      ],
      "evolvesFrom": [
        184
      ],
      "evolvesTo": [
        186
      ],
      "stats": {
        "hp": 60,
        "atk": 40,
        "def": 60,
        "spd": 55,
        "spa": 95,
        "spdef": 60
      },
      "bst": 370,
      "statsSource": "Pokerex"
    },
    {
      "number": 186,
      "name": "Chandelure",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/chandelure.png",
      "types": [
        "ghost",
        "fire"
      ],
      "evolvesFrom": [
        185
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 55,
        "def": 90,
        "spd": 80,
        "spa": 145,
        "spdef": 90
      },
      "bst": 520,
      "statsSource": "Pokerex"
    },
    {
      "number": 187,
      "name": "Pancham",
      "region": "",
      "location": "Ivy River",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pancham.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        188
      ],
      "stats": {
        "hp": 67,
        "atk": 82,
        "def": 62,
        "spd": 43,
        "spa": 46,
        "spdef": 48
      },
      "bst": 348,
      "statsSource": "Pokerex"
    },
    {
      "number": 188,
      "name": "Pangoro",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pangoro.png",
      "types": [
        "fighting",
        "dark"
      ],
      "evolvesFrom": [
        187
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 124,
        "def": 78,
        "spd": 58,
        "spa": 69,
        "spdef": 71
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 189,
      "name": "Oranguru",
      "region": "",
      "location": "Ivy Forest",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/oranguru.png",
      "types": [
        "normal",
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 90,
        "atk": 60,
        "def": 80,
        "spd": 60,
        "spa": 90,
        "spdef": 110
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 190,
      "name": "Passimian",
      "region": "",
      "location": "Ivy Forest",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/passimian.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 100,
        "atk": 120,
        "def": 90,
        "spd": 80,
        "spa": 40,
        "spdef": 60
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 191,
      "name": "Ralts",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/ralts.png",
      "types": [
        "psychic",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        192
      ],
      "stats": {
        "hp": 28,
        "atk": 25,
        "def": 25,
        "spd": 40,
        "spa": 45,
        "spdef": 35
      },
      "bst": 198,
      "statsSource": "Pokerex"
    },
    {
      "number": 192,
      "name": "Kirlia",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/kirlia.png",
      "types": [
        "psychic",
        "fairy"
      ],
      "evolvesFrom": [
        191
      ],
      "evolvesTo": [
        193,
        194
      ],
      "stats": {
        "hp": 38,
        "atk": 35,
        "def": 35,
        "spd": 50,
        "spa": 65,
        "spdef": 55
      },
      "bst": 278,
      "statsSource": "Pokerex"
    },
    {
      "number": 193,
      "name": "Gardevoir",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/gardevoir.png",
      "types": [
        "psychic",
        "fairy"
      ],
      "evolvesFrom": [
        192
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 68,
        "atk": 65,
        "def": 65,
        "spd": 80,
        "spa": 125,
        "spdef": 115
      },
      "bst": 518,
      "statsSource": "Pokerex"
    },
    {
      "number": 194,
      "name": "Gallade",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/gallade.png",
      "types": [
        "psychic",
        "fighting"
      ],
      "evolvesFrom": [
        192
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 68,
        "atk": 125,
        "def": 65,
        "spd": 80,
        "spa": 65,
        "spdef": 115
      },
      "bst": 518,
      "statsSource": "Pokerex"
    },
    {
      "number": 195,
      "name": "Charcadet",
      "region": "",
      "location": "Galecrest City",
      "rarity": "Unique",
      "notes": "Get a fake ID made from the Somber HQ in Silversun",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-935.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        196,
        197
      ],
      "stats": {
        "hp": 40,
        "atk": 50,
        "def": 40,
        "spd": 35,
        "spa": 50,
        "spdef": 40
      },
      "bst": 255,
      "statsSource": "Pokerex"
    },
    {
      "number": 196,
      "name": "Ceruledge",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-937.png",
      "types": [
        "fire",
        "ghost"
      ],
      "evolvesFrom": [
        195
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 125,
        "def": 80,
        "spd": 85,
        "spa": 60,
        "spdef": 100
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 197,
      "name": "Armarouge",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-936.png",
      "types": [
        "fire",
        "psychic"
      ],
      "evolvesFrom": [
        195
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 60,
        "def": 100,
        "spd": 75,
        "spa": 125,
        "spdef": 80
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 198,
      "name": "Mareep",
      "region": "",
      "location": "Route 3",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/mareep.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        199
      ],
      "stats": {
        "hp": 55,
        "atk": 40,
        "def": 40,
        "spd": 35,
        "spa": 65,
        "spdef": 45
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 199,
      "name": "Flaaffy",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/flaaffy.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [
        198
      ],
      "evolvesTo": [
        200
      ],
      "stats": {
        "hp": 70,
        "atk": 55,
        "def": 55,
        "spd": 45,
        "spa": 80,
        "spdef": 60
      },
      "bst": 365,
      "statsSource": "Pokerex"
    },
    {
      "number": 200,
      "name": "Ampharos",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/ampharos.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [
        199
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 90,
        "atk": 75,
        "def": 85,
        "spd": 55,
        "spa": 115,
        "spdef": 90
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 201,
      "name": "Varoom",
      "region": "",
      "location": "Sewers",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-965.png",
      "types": [
        "steel",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        202
      ],
      "stats": {
        "hp": 45,
        "atk": 70,
        "def": 63,
        "spd": 47,
        "spa": 30,
        "spdef": 45
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 202,
      "name": "Revavroom",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-966.png",
      "types": [
        "steel",
        "poison"
      ],
      "evolvesFrom": [
        201
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 119,
        "def": 90,
        "spd": 90,
        "spa": 54,
        "spdef": 67
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 203,
      "name": "Finneon",
      "region": "",
      "location": "Various",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/finneon.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        204
      ],
      "stats": {
        "hp": 49,
        "atk": 49,
        "def": 56,
        "spd": 66,
        "spa": 49,
        "spdef": 61
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 204,
      "name": "Lumineon",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/lumineon.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        203
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 69,
        "atk": 69,
        "def": 76,
        "spd": 91,
        "spa": 69,
        "spdef": 86
      },
      "bst": 460,
      "statsSource": "Pokerex"
    },
    {
      "number": 205,
      "name": "Skarmory",
      "region": "",
      "location": "Route 3",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/skarmory.png",
      "types": [
        "steel",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 65,
        "atk": 80,
        "def": 140,
        "spd": 70,
        "spa": 40,
        "spdef": 70
      },
      "bst": 465,
      "statsSource": "Pokerex"
    },
    {
      "number": 206,
      "name": "Honedge",
      "region": "",
      "location": "Route 5",
      "rarity": "Unique",
      "notes": "Offered by Greta the traveling merchant (Mysterious Egg)",
      "availability": "Available",
      "sprite": "assets/sprites/honedge.png",
      "types": [
        "steel",
        "ghost"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        207
      ],
      "stats": {
        "hp": 45,
        "atk": 80,
        "def": 100,
        "spd": 28,
        "spa": 35,
        "spdef": 37
      },
      "bst": 325,
      "statsSource": "Pokerex"
    },
    {
      "number": 207,
      "name": "Doublade",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/doublade.png",
      "types": [
        "steel",
        "ghost"
      ],
      "evolvesFrom": [
        206
      ],
      "evolvesTo": [
        208
      ],
      "stats": {
        "hp": 59,
        "atk": 110,
        "def": 150,
        "spd": 35,
        "spa": 45,
        "spdef": 49
      },
      "bst": 448,
      "statsSource": "Pokerex"
    },
    {
      "number": 208,
      "name": "Aegislash",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/aegislash.png",
      "types": [
        "steel",
        "ghost"
      ],
      "evolvesFrom": [
        207
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 50,
        "def": 140,
        "spd": 60,
        "spa": 50,
        "spdef": 140
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 209,
      "name": "Scyther",
      "region": "",
      "location": "Ivy River",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/scyther.png",
      "types": [
        "bug",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        210,
        211
      ],
      "stats": {
        "hp": 70,
        "atk": 110,
        "def": 80,
        "spd": 105,
        "spa": 55,
        "spdef": 80
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 210,
      "name": "Scizor",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/scizor.png",
      "types": [
        "bug",
        "steel"
      ],
      "evolvesFrom": [
        209
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 130,
        "def": 100,
        "spd": 65,
        "spa": 55,
        "spdef": 80
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 211,
      "name": "Kleavor",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/kleavor.png",
      "types": [
        "bug",
        "rock"
      ],
      "evolvesFrom": [
        209
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 135,
        "def": 95,
        "spd": 85,
        "spa": 45,
        "spdef": 70
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 212,
      "name": "Kangaskhan",
      "region": "",
      "location": "Route 6 Ranger Institute",
      "rarity": "Unique",
      "notes": "Get promoted to Associate Ranger first",
      "availability": "Available",
      "sprite": "assets/sprites/kangaskhan.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 105,
        "atk": 95,
        "def": 80,
        "spd": 90,
        "spa": 40,
        "spdef": 80
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 213,
      "name": "Pinsir",
      "region": "",
      "location": "Rivetshore Ranger Institute",
      "rarity": "Unique",
      "notes": "Trade",
      "availability": "Available",
      "sprite": "assets/sprites/pinsir.png",
      "types": [
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 65,
        "atk": 125,
        "def": 100,
        "spd": 85,
        "spa": 55,
        "spdef": 70
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 214,
      "name": "Furfrou",
      "region": "",
      "location": "Route 7",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/furfrou.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 80,
        "def": 60,
        "spd": 102,
        "spa": 65,
        "spdef": 90
      },
      "bst": 472,
      "statsSource": "Pokerex"
    },
    {
      "number": 215,
      "name": "Bombirdier",
      "region": "",
      "location": "Ivy River",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-962.png",
      "types": [
        "flying",
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 103,
        "def": 85,
        "spd": 82,
        "spa": 60,
        "spdef": 85
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 216,
      "name": "Arrokuda",
      "region": "",
      "location": "Route 6",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/arrokuda.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        217
      ],
      "stats": {
        "hp": 41,
        "atk": 63,
        "def": 40,
        "spd": 66,
        "spa": 40,
        "spdef": 30
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 217,
      "name": "Barraskewda",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/barraskewda.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        216
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 61,
        "atk": 123,
        "def": 60,
        "spd": 136,
        "spa": 60,
        "spdef": 50
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 218,
      "name": "Bruxish",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/bruxish.png",
      "types": [
        "water",
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 68,
        "atk": 105,
        "def": 70,
        "spd": 92,
        "spa": 70,
        "spdef": 70
      },
      "bst": 475,
      "statsSource": "Pokerex"
    },
    {
      "number": 219,
      "name": "Carvanha",
      "region": "",
      "location": "Ivy River",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/carvanha.png",
      "types": [
        "water",
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        220
      ],
      "stats": {
        "hp": 45,
        "atk": 90,
        "def": 20,
        "spd": 65,
        "spa": 65,
        "spdef": 20
      },
      "bst": 305,
      "statsSource": "Pokerex"
    },
    {
      "number": 220,
      "name": "Sharpedo",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/sharpedo.png",
      "types": [
        "water",
        "dark"
      ],
      "evolvesFrom": [
        219
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 120,
        "def": 40,
        "spd": 95,
        "spa": 95,
        "spdef": 40
      },
      "bst": 460,
      "statsSource": "Pokerex"
    },
    {
      "number": 221,
      "name": "Slowpoke",
      "region": "Galar",
      "location": "Lily Pond",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/slowpoke-galar.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        222,
        223
      ],
      "stats": {
        "hp": 90,
        "atk": 65,
        "def": 65,
        "spd": 15,
        "spa": 40,
        "spdef": 40
      },
      "bst": 315,
      "statsSource": "Pokerex"
    },
    {
      "number": 222,
      "name": "Slowbro",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/slowbro-galar.png",
      "types": [
        "poison",
        "psychic"
      ],
      "evolvesFrom": [
        221
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 100,
        "def": 95,
        "spd": 30,
        "spa": 100,
        "spdef": 70
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 223,
      "name": "Slowking",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/slowking-galar.png",
      "types": [
        "poison",
        "psychic"
      ],
      "evolvesFrom": [
        221
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 65,
        "def": 80,
        "spd": 30,
        "spa": 110,
        "spdef": 110
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 224,
      "name": "Oshawott",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/oshawott.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        225
      ],
      "stats": {
        "hp": 55,
        "atk": 55,
        "def": 45,
        "spd": 45,
        "spa": 63,
        "spdef": 45
      },
      "bst": 308,
      "statsSource": "Pokerex"
    },
    {
      "number": 225,
      "name": "Dewott",
      "region": "",
      "location": "Route 3 Caverns",
      "rarity": "Uncommon",
      "notes": "Trolling Rod",
      "availability": "Available",
      "sprite": "assets/sprites/dewott.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        224
      ],
      "evolvesTo": [
        226
      ],
      "stats": {
        "hp": 75,
        "atk": 75,
        "def": 60,
        "spd": 60,
        "spa": 83,
        "spdef": 60
      },
      "bst": 413,
      "statsSource": "Pokerex"
    },
    {
      "number": 226,
      "name": "Samurott",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/samurott.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        225
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 100,
        "def": 85,
        "spd": 70,
        "spa": 108,
        "spdef": 70
      },
      "bst": 528,
      "statsSource": "Pokerex"
    },
    {
      "number": 227,
      "name": "Tynamo",
      "region": "",
      "location": "Ivy River",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/tynamo.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        228
      ],
      "stats": {
        "hp": 35,
        "atk": 55,
        "def": 40,
        "spd": 60,
        "spa": 45,
        "spdef": 40
      },
      "bst": 275,
      "statsSource": "Pokerex"
    },
    {
      "number": 228,
      "name": "Eelektrik",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/eelektrik.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [
        227
      ],
      "evolvesTo": [
        229
      ],
      "stats": {
        "hp": 65,
        "atk": 85,
        "def": 70,
        "spd": 40,
        "spa": 75,
        "spdef": 70
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 229,
      "name": "Eelektross",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/eelektross.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [
        228
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 85,
        "atk": 115,
        "def": 80,
        "spd": 50,
        "spa": 105,
        "spdef": 80
      },
      "bst": 515,
      "statsSource": "Pokerex"
    },
    {
      "number": 230,
      "name": "Corsola",
      "region": "",
      "location": "Rivetshore City",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/corsola.png",
      "types": [
        "water",
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        231
      ],
      "stats": {
        "hp": 65,
        "atk": 55,
        "def": 95,
        "spd": 35,
        "spa": 65,
        "spdef": 95
      },
      "bst": 410,
      "statsSource": "Pokerex"
    },
    {
      "number": 231,
      "name": "Cursola",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/cursola.png",
      "types": [
        "ghost"
      ],
      "evolvesFrom": [
        230
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 95,
        "def": 50,
        "spd": 30,
        "spa": 145,
        "spdef": 130
      },
      "bst": 510,
      "statsSource": "Pokerex"
    },
    {
      "number": 232,
      "name": "Mareanie",
      "region": "",
      "location": "Rivetshore City",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/mareanie.png",
      "types": [
        "poison",
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        233
      ],
      "stats": {
        "hp": 50,
        "atk": 53,
        "def": 62,
        "spd": 45,
        "spa": 43,
        "spdef": 52
      },
      "bst": 305,
      "statsSource": "Pokerex"
    },
    {
      "number": 233,
      "name": "Toxapex",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/toxapex.png",
      "types": [
        "poison",
        "water"
      ],
      "evolvesFrom": [
        232
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 63,
        "def": 152,
        "spd": 35,
        "spa": 53,
        "spdef": 142
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 234,
      "name": "Voltorb",
      "region": "Hisui",
      "location": "Static Cave",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/voltorb-hisui.png",
      "types": [
        "electric",
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        235
      ],
      "stats": {
        "hp": 40,
        "atk": 30,
        "def": 50,
        "spd": 100,
        "spa": 55,
        "spdef": 55
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 235,
      "name": "Electrode",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/electrode-hisui.png",
      "types": [
        "electric",
        "grass"
      ],
      "evolvesFrom": [
        234
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 50,
        "def": 70,
        "spd": 150,
        "spa": 80,
        "spdef": 80
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 236,
      "name": "Electrike",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/electrike.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        237
      ],
      "stats": {
        "hp": 40,
        "atk": 45,
        "def": 40,
        "spd": 65,
        "spa": 65,
        "spdef": 40
      },
      "bst": 295,
      "statsSource": "Pokerex"
    },
    {
      "number": 237,
      "name": "Manectric",
      "region": "",
      "location": "Route 6 Ranger Institute",
      "rarity": "Unique",
      "notes": "Get promoted to Associate Ranger first",
      "availability": "Available",
      "sprite": "assets/sprites/manectric.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [
        236
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 75,
        "def": 60,
        "spd": 105,
        "spa": 105,
        "spdef": 60
      },
      "bst": 475,
      "statsSource": "Pokerex"
    },
    {
      "number": 238,
      "name": "Yamper",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/yamper.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        239
      ],
      "stats": {
        "hp": 59,
        "atk": 45,
        "def": 50,
        "spd": 26,
        "spa": 40,
        "spdef": 50
      },
      "bst": 270,
      "statsSource": "Pokerex"
    },
    {
      "number": 239,
      "name": "Boltund",
      "region": "",
      "location": "Route 7",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/boltund.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [
        238
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 69,
        "atk": 90,
        "def": 60,
        "spd": 121,
        "spa": 90,
        "spdef": 60
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 240,
      "name": "Geodude",
      "region": "Alola",
      "location": "Static Cave",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/geodude-alola.png",
      "types": [
        "rock",
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        241
      ],
      "stats": {
        "hp": 40,
        "atk": 80,
        "def": 100,
        "spd": 20,
        "spa": 30,
        "spdef": 30
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 241,
      "name": "Graveler",
      "region": "Alola",
      "location": "Static Cave",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/graveler-alola.png",
      "types": [
        "rock",
        "electric"
      ],
      "evolvesFrom": [
        240
      ],
      "evolvesTo": [
        242
      ],
      "stats": {
        "hp": 55,
        "atk": 95,
        "def": 115,
        "spd": 35,
        "spa": 45,
        "spdef": 45
      },
      "bst": 390,
      "statsSource": "Pokerex"
    },
    {
      "number": 242,
      "name": "Golem",
      "region": "Alola",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/golem-alola.png",
      "types": [
        "rock",
        "electric"
      ],
      "evolvesFrom": [
        241
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 120,
        "def": 130,
        "spd": 45,
        "spa": 55,
        "spdef": 65
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 243,
      "name": "Houndour",
      "region": "",
      "location": "Pelluca City",
      "rarity": "Unique",
      "notes": "Trade",
      "availability": "Available",
      "sprite": "assets/sprites/houndour.png",
      "types": [
        "dark",
        "fire"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        244
      ],
      "stats": {
        "hp": 45,
        "atk": 60,
        "def": 30,
        "spd": 65,
        "spa": 80,
        "spdef": 50
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 244,
      "name": "Houndoom",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/houndoom.png",
      "types": [
        "dark",
        "fire"
      ],
      "evolvesFrom": [
        243
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 90,
        "def": 50,
        "spd": 95,
        "spa": 110,
        "spdef": 80
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 245,
      "name": "Ponyta",
      "region": "Galar",
      "location": "Ceram Base Camp",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/ponyta-galar.png",
      "types": [
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        246
      ],
      "stats": {
        "hp": 50,
        "atk": 85,
        "def": 55,
        "spd": 90,
        "spa": 65,
        "spdef": 65
      },
      "bst": 410,
      "statsSource": "Pokerex"
    },
    {
      "number": 246,
      "name": "Rapidash",
      "region": "Galar",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/rapidash-galar.png",
      "types": [
        "psychic",
        "fairy"
      ],
      "evolvesFrom": [
        245
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 65,
        "atk": 100,
        "def": 70,
        "spd": 105,
        "spa": 80,
        "spdef": 80
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 247,
      "name": "Wooloo",
      "region": "",
      "location": "Route 7",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/wooloo.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        248
      ],
      "stats": {
        "hp": 42,
        "atk": 40,
        "def": 55,
        "spd": 48,
        "spa": 40,
        "spdef": 45
      },
      "bst": 270,
      "statsSource": "Pokerex"
    },
    {
      "number": 248,
      "name": "Dubwool",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/dubwool.png",
      "types": [
        "normal"
      ],
      "evolvesFrom": [
        247
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 72,
        "atk": 80,
        "def": 100,
        "spd": 88,
        "spa": 60,
        "spdef": 90
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 249,
      "name": "Wingull",
      "region": "",
      "location": "Pelluca City",
      "rarity": "Common",
      "notes": "Talk to the girl throwing stones at the Wingulls",
      "availability": "Available",
      "sprite": "assets/sprites/wingull.png",
      "types": [
        "water",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        250
      ],
      "stats": {
        "hp": 40,
        "atk": 30,
        "def": 30,
        "spd": 85,
        "spa": 55,
        "spdef": 30
      },
      "bst": 270,
      "statsSource": "Pokerex"
    },
    {
      "number": 250,
      "name": "Pelipper",
      "region": "",
      "location": "Route 2",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pelipper.png",
      "types": [
        "water",
        "flying"
      ],
      "evolvesFrom": [
        249
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 50,
        "def": 100,
        "spd": 65,
        "spa": 95,
        "spdef": 70
      },
      "bst": 440,
      "statsSource": "Pokerex"
    },
    {
      "number": 251,
      "name": "Goomy",
      "region": "",
      "location": "Route 6",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/goomy.png",
      "types": [
        "dragon"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        252
      ],
      "stats": {
        "hp": 45,
        "atk": 50,
        "def": 35,
        "spd": 40,
        "spa": 55,
        "spdef": 75
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 252,
      "name": "Sliggoo",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/sliggoo-hisui.png",
      "types": [
        "steel",
        "dragon"
      ],
      "evolvesFrom": [
        251
      ],
      "evolvesTo": [
        253
      ],
      "stats": {
        "hp": 58,
        "atk": 75,
        "def": 83,
        "spd": 40,
        "spa": 83,
        "spdef": 113
      },
      "bst": 452,
      "statsSource": "Pokerex"
    },
    {
      "number": 253,
      "name": "Goodra",
      "region": "Hisui",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/goodra-hisui.png",
      "types": [
        "steel",
        "dragon"
      ],
      "evolvesFrom": [
        252
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 100,
        "def": 100,
        "spd": 60,
        "spa": 110,
        "spdef": 150
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 254,
      "name": "Squirtle",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/squirtle.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        255
      ],
      "stats": {
        "hp": 44,
        "atk": 48,
        "def": 65,
        "spd": 43,
        "spa": 50,
        "spdef": 64
      },
      "bst": 314,
      "statsSource": "Pokerex"
    },
    {
      "number": 255,
      "name": "Wartortle",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/wartortle.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        254
      ],
      "evolvesTo": [
        256
      ],
      "stats": {
        "hp": 59,
        "atk": 63,
        "def": 80,
        "spd": 58,
        "spa": 65,
        "spdef": 80
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 256,
      "name": "Blastoise",
      "region": "",
      "location": "Lily Pond",
      "rarity": "Unique",
      "notes": "Wake it up with a Pokeflute",
      "availability": "Available",
      "sprite": "assets/sprites/blastoise.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        255
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 79,
        "atk": 83,
        "def": 100,
        "spd": 78,
        "spa": 85,
        "spdef": 105
      },
      "bst": 530,
      "statsSource": "Pokerex"
    },
    {
      "number": 257,
      "name": "Charmander",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/charmander.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        258
      ],
      "stats": {
        "hp": 39,
        "atk": 52,
        "def": 43,
        "spd": 65,
        "spa": 60,
        "spdef": 50
      },
      "bst": 309,
      "statsSource": "Pokerex"
    },
    {
      "number": 258,
      "name": "Charmeleon",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/charmeleon.png",
      "types": [
        "fire"
      ],
      "evolvesFrom": [
        257
      ],
      "evolvesTo": [
        259
      ],
      "stats": {
        "hp": 58,
        "atk": 64,
        "def": 58,
        "spd": 80,
        "spa": 80,
        "spdef": 65
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 259,
      "name": "Charizard",
      "region": "",
      "location": "Mt Ceram",
      "rarity": "Unique",
      "notes": "Wake it up with a Pokeflute",
      "availability": "Available",
      "sprite": "assets/sprites/charizard.png",
      "types": [
        "fire",
        "flying"
      ],
      "evolvesFrom": [
        258
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 78,
        "atk": 84,
        "def": 78,
        "spd": 100,
        "spa": 109,
        "spdef": 85
      },
      "bst": 534,
      "statsSource": "Pokerex"
    },
    {
      "number": 260,
      "name": "Bulbasaur",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/bulbasaur.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        261
      ],
      "stats": {
        "hp": 45,
        "atk": 49,
        "def": 49,
        "spd": 45,
        "spa": 65,
        "spdef": 65
      },
      "bst": 318,
      "statsSource": "Pokerex"
    },
    {
      "number": 261,
      "name": "Ivysaur",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/ivysaur.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [
        260
      ],
      "evolvesTo": [
        262
      ],
      "stats": {
        "hp": 60,
        "atk": 62,
        "def": 63,
        "spd": 60,
        "spa": 80,
        "spdef": 80
      },
      "bst": 405,
      "statsSource": "Pokerex"
    },
    {
      "number": 262,
      "name": "Venusaur",
      "region": "",
      "location": "Route 6",
      "rarity": "Unique",
      "notes": "Wake it up with a Pokeflute",
      "availability": "Available",
      "sprite": "assets/sprites/venusaur.png",
      "types": [
        "grass",
        "poison"
      ],
      "evolvesFrom": [
        261
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 82,
        "def": 83,
        "spd": 80,
        "spa": 100,
        "spdef": 100
      },
      "bst": 525,
      "statsSource": "Pokerex"
    },
    {
      "number": 263,
      "name": "Jangmo_o",
      "region": "",
      "location": "Galecrest City",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/jangmo-o.png",
      "types": [
        "dragon"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        264
      ],
      "stats": {
        "hp": 45,
        "atk": 55,
        "def": 65,
        "spd": 45,
        "spa": 45,
        "spdef": 45
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 264,
      "name": "Hakamo_o",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/hakamo-o.png",
      "types": [
        "dragon",
        "fighting"
      ],
      "evolvesFrom": [
        263
      ],
      "evolvesTo": [
        265
      ],
      "stats": {
        "hp": 55,
        "atk": 75,
        "def": 90,
        "spd": 65,
        "spa": 65,
        "spdef": 70
      },
      "bst": 420,
      "statsSource": "Pokerex"
    },
    {
      "number": 265,
      "name": "Kommo_o",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/kommo-o.png",
      "types": [
        "dragon",
        "fighting"
      ],
      "evolvesFrom": [
        264
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 110,
        "def": 125,
        "spd": 85,
        "spa": 100,
        "spdef": 105
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 266,
      "name": "Snom",
      "region": "",
      "location": "Mirroh Peak",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/snom.png",
      "types": [
        "ice",
        "bug"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        267
      ],
      "stats": {
        "hp": 30,
        "atk": 25,
        "def": 35,
        "spd": 20,
        "spa": 45,
        "spdef": 30
      },
      "bst": 185,
      "statsSource": "Pokerex"
    },
    {
      "number": 267,
      "name": "Frosmoth",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/frosmoth.png",
      "types": [
        "ice",
        "bug"
      ],
      "evolvesFrom": [
        266
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 65,
        "def": 60,
        "spd": 65,
        "spa": 125,
        "spdef": 90
      },
      "bst": 475,
      "statsSource": "Pokerex"
    },
    {
      "number": 268,
      "name": "Wattrel",
      "region": "",
      "location": "Route 5",
      "rarity": "Uncommon",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-940.png",
      "types": [
        "electric",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        269
      ],
      "stats": {
        "hp": 40,
        "atk": 40,
        "def": 35,
        "spd": 70,
        "spa": 55,
        "spdef": 40
      },
      "bst": 280,
      "statsSource": "Pokerex"
    },
    {
      "number": 269,
      "name": "Kilowattrel",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/pokeapi-941.png",
      "types": [
        "electric",
        "flying"
      ],
      "evolvesFrom": [
        268
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 70,
        "def": 60,
        "spd": 125,
        "spa": 105,
        "spdef": 60
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 270,
      "name": "Beldum",
      "region": "",
      "location": "Mirroh Interior B1F",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/beldum.png",
      "types": [
        "steel",
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        271
      ],
      "stats": {
        "hp": 40,
        "atk": 55,
        "def": 80,
        "spd": 30,
        "spa": 35,
        "spdef": 60
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 271,
      "name": "Metang",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/metang.png",
      "types": [
        "steel",
        "psychic"
      ],
      "evolvesFrom": [
        270
      ],
      "evolvesTo": [
        272
      ],
      "stats": {
        "hp": 60,
        "atk": 75,
        "def": 100,
        "spd": 50,
        "spa": 55,
        "spdef": 80
      },
      "bst": 420,
      "statsSource": "Pokerex"
    },
    {
      "number": 272,
      "name": "Metagross",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/metagross.png",
      "types": [
        "steel",
        "psychic"
      ],
      "evolvesFrom": [
        271
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 135,
        "def": 130,
        "spd": 70,
        "spa": 95,
        "spdef": 90
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 273,
      "name": "Absol",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/absol.png",
      "types": [
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 65,
        "atk": 130,
        "def": 60,
        "spd": 75,
        "spa": 75,
        "spdef": 60
      },
      "bst": 465,
      "statsSource": "Pokerex"
    },
    {
      "number": 274,
      "name": "Tauros",
      "region": "Paldea",
      "location": "Route 6 Ranger Institute",
      "rarity": "Unique",
      "notes": "Paldea Blaze. Get promoted to Associate Ranger first",
      "availability": "Available",
      "sprite": "assets/sprites/tauros.png",
      "types": [
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 110,
        "def": 105,
        "spd": 100,
        "spa": 30,
        "spdef": 70
      },
      "bst": 490,
      "statsSource": "Pokerex"
    },
    {
      "number": 275,
      "name": "Surskit",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "Breeding",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/surskit.png",
      "types": [
        "bug",
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        276
      ],
      "stats": {
        "hp": 40,
        "atk": 30,
        "def": 32,
        "spd": 65,
        "spa": 50,
        "spdef": 52
      },
      "bst": 269,
      "statsSource": "Pokerex"
    },
    {
      "number": 276,
      "name": "Masquerain",
      "region": "",
      "location": "Lily Pond",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/masquerain.png",
      "types": [
        "bug",
        "flying"
      ],
      "evolvesFrom": [
        275
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 60,
        "def": 62,
        "spd": 80,
        "spa": 100,
        "spdef": 82
      },
      "bst": 454,
      "statsSource": "Pokerex"
    },
    {
      "number": 277,
      "name": "Clauncher",
      "region": "",
      "location": "Pelluca City",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/clauncher.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        278
      ],
      "stats": {
        "hp": 50,
        "atk": 53,
        "def": 62,
        "spd": 44,
        "spa": 58,
        "spdef": 63
      },
      "bst": 330,
      "statsSource": "Pokerex"
    },
    {
      "number": 278,
      "name": "Clawitzer",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/clawitzer.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [
        277
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 71,
        "atk": 73,
        "def": 88,
        "spd": 59,
        "spa": 120,
        "spdef": 89
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 279,
      "name": "Alomomola",
      "region": "",
      "location": "Lily Pond",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/alomomola.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 165,
        "atk": 75,
        "def": 80,
        "spd": 65,
        "spa": 40,
        "spdef": 45
      },
      "bst": 470,
      "statsSource": "Pokerex"
    },
    {
      "number": 280,
      "name": "Magikarp",
      "region": "",
      "location": "Various",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/magikarp.png",
      "types": [
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        281
      ],
      "stats": {
        "hp": 20,
        "atk": 10,
        "def": 55,
        "spd": 80,
        "spa": 15,
        "spdef": 20
      },
      "bst": 200,
      "statsSource": "Pokerex"
    },
    {
      "number": 281,
      "name": "Gyarados",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/gyarados.png",
      "types": [
        "water",
        "flying"
      ],
      "evolvesFrom": [
        280
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 95,
        "atk": 125,
        "def": 79,
        "spd": 81,
        "spa": 60,
        "spdef": 100
      },
      "bst": 540,
      "statsSource": "Pokerex"
    },
    {
      "number": 282,
      "name": "Spiritomb",
      "region": "",
      "location": "Mirroh Base Camp Graveyard",
      "rarity": "Unique",
      "notes": "Greta sells a stone in Route 5. Place it on the empty patch.",
      "availability": "Available",
      "sprite": "assets/sprites/spiritomb.png",
      "types": [
        "ghost",
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 92,
        "def": 108,
        "spd": 35,
        "spa": 92,
        "spdef": 108
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 283,
      "name": "Baltoy",
      "region": "",
      "location": "Mt. Mirroh Interior 1F",
      "rarity": "Rare",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/baltoy.png",
      "types": [
        "ground",
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        284
      ],
      "stats": {
        "hp": 40,
        "atk": 40,
        "def": 55,
        "spd": 55,
        "spa": 40,
        "spdef": 70
      },
      "bst": 300,
      "statsSource": "Pokerex"
    },
    {
      "number": 284,
      "name": "Claydol",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/claydol.png",
      "types": [
        "ground",
        "psychic"
      ],
      "evolvesFrom": [
        283
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 70,
        "def": 105,
        "spd": 75,
        "spa": 70,
        "spdef": 120
      },
      "bst": 500,
      "statsSource": "Pokerex"
    },
    {
      "number": 285,
      "name": "Dhelmise",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/dhelmise.png",
      "types": [
        "ghost",
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 131,
        "def": 100,
        "spd": 40,
        "spa": 86,
        "spdef": 90
      },
      "bst": 517,
      "statsSource": "Pokerex"
    },
    {
      "number": 286,
      "name": "Mimikyu",
      "region": "",
      "location": "",
      "rarity": "Unique",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/mimikyu.png",
      "types": [
        "ghost",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 55,
        "atk": 90,
        "def": 80,
        "spd": 96,
        "spa": 50,
        "spdef": 105
      },
      "bst": 476,
      "statsSource": "Pokerex"
    },
    {
      "number": 287,
      "name": "Lileep",
      "region": "",
      "location": "Ancient Ceram",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/lileep.png",
      "types": [
        "rock",
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        288
      ],
      "stats": {
        "hp": 66,
        "atk": 41,
        "def": 77,
        "spd": 23,
        "spa": 61,
        "spdef": 87
      },
      "bst": 355,
      "statsSource": "Pokerex"
    },
    {
      "number": 288,
      "name": "Cradily",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/cradily.png",
      "types": [
        "rock",
        "grass"
      ],
      "evolvesFrom": [
        287
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 86,
        "atk": 81,
        "def": 97,
        "spd": 43,
        "spa": 81,
        "spdef": 107
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 289,
      "name": "Archen",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/archen.png",
      "types": [
        "rock",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        290
      ],
      "stats": {
        "hp": 55,
        "atk": 112,
        "def": 45,
        "spd": 70,
        "spa": 74,
        "spdef": 45
      },
      "bst": 401,
      "statsSource": "Pokerex"
    },
    {
      "number": 290,
      "name": "Archeops",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/archeops.png",
      "types": [
        "rock",
        "flying"
      ],
      "evolvesFrom": [
        289
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 75,
        "atk": 140,
        "def": 65,
        "spd": 110,
        "spa": 112,
        "spdef": 65
      },
      "bst": 567,
      "statsSource": "Pokerex"
    },
    {
      "number": 291,
      "name": "Amaura",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/amaura.png",
      "types": [
        "rock",
        "ice"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        292
      ],
      "stats": {
        "hp": 77,
        "atk": 59,
        "def": 50,
        "spd": 46,
        "spa": 67,
        "spdef": 63
      },
      "bst": 362,
      "statsSource": "Pokerex"
    },
    {
      "number": 292,
      "name": "Aurorus",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/aurorus.png",
      "types": [
        "rock",
        "ice"
      ],
      "evolvesFrom": [
        291
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 123,
        "atk": 77,
        "def": 72,
        "spd": 58,
        "spa": 99,
        "spdef": 92
      },
      "bst": 521,
      "statsSource": "Pokerex"
    },
    {
      "number": 293,
      "name": "Shieldon",
      "region": "",
      "location": "Ancient Ceram",
      "rarity": "Common",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/shieldon.png",
      "types": [
        "rock",
        "steel"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        294
      ],
      "stats": {
        "hp": 30,
        "atk": 42,
        "def": 118,
        "spd": 30,
        "spa": 42,
        "spdef": 88
      },
      "bst": 350,
      "statsSource": "Pokerex"
    },
    {
      "number": 294,
      "name": "Bastiodon",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/bastiodon.png",
      "types": [
        "rock",
        "steel"
      ],
      "evolvesFrom": [
        293
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 52,
        "def": 168,
        "spd": 30,
        "spa": 47,
        "spdef": 138
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 295,
      "name": "Tyrunt",
      "region": "",
      "location": "Ancient Ceram",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/tyrunt.png",
      "types": [
        "rock",
        "dragon"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        296
      ],
      "stats": {
        "hp": 58,
        "atk": 89,
        "def": 77,
        "spd": 48,
        "spa": 45,
        "spdef": 45
      },
      "bst": 362,
      "statsSource": "Pokerex"
    },
    {
      "number": 296,
      "name": "Tyrantrum",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/tyrantrum.png",
      "types": [
        "rock",
        "dragon"
      ],
      "evolvesFrom": [
        295
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 82,
        "atk": 121,
        "def": 119,
        "spd": 71,
        "spa": 69,
        "spdef": 59
      },
      "bst": 521,
      "statsSource": "Pokerex"
    },
    {
      "number": 297,
      "name": "Omanyte",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/omanyte.png",
      "types": [
        "rock",
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        298
      ],
      "stats": {
        "hp": 35,
        "atk": 40,
        "def": 100,
        "spd": 35,
        "spa": 90,
        "spdef": 55
      },
      "bst": 355,
      "statsSource": "Pokerex"
    },
    {
      "number": 298,
      "name": "Omastar",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/omastar.png",
      "types": [
        "rock",
        "water"
      ],
      "evolvesFrom": [
        297
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 70,
        "atk": 60,
        "def": 125,
        "spd": 55,
        "spa": 115,
        "spdef": 70
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 299,
      "name": "Kabuto",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/kabuto.png",
      "types": [
        "rock",
        "water"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        300
      ],
      "stats": {
        "hp": 30,
        "atk": 80,
        "def": 90,
        "spd": 55,
        "spa": 55,
        "spdef": 45
      },
      "bst": 355,
      "statsSource": "Pokerex"
    },
    {
      "number": 300,
      "name": "Kabutops",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/kabutops.png",
      "types": [
        "rock",
        "water"
      ],
      "evolvesFrom": [
        299
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 60,
        "atk": 115,
        "def": 105,
        "spd": 80,
        "spa": 65,
        "spdef": 70
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 301,
      "name": "Cranidos",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/cranidos.png",
      "types": [
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        302
      ],
      "stats": {
        "hp": 67,
        "atk": 125,
        "def": 40,
        "spd": 58,
        "spa": 30,
        "spdef": 30
      },
      "bst": 350,
      "statsSource": "Pokerex"
    },
    {
      "number": 302,
      "name": "Rampardos",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/rampardos.png",
      "types": [
        "rock"
      ],
      "evolvesFrom": [
        301
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 97,
        "atk": 165,
        "def": 60,
        "spd": 58,
        "spa": 65,
        "spdef": 50
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 303,
      "name": "Relicanth",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/relicanth.png",
      "types": [
        "water",
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 100,
        "atk": 90,
        "def": 130,
        "spd": 55,
        "spa": 45,
        "spdef": 65
      },
      "bst": 485,
      "statsSource": "Pokerex"
    },
    {
      "number": 304,
      "name": "Tirtouga",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/tirtouga.png",
      "types": [
        "water",
        "rock"
      ],
      "evolvesFrom": [],
      "evolvesTo": [
        305
      ],
      "stats": {
        "hp": 54,
        "atk": 78,
        "def": 103,
        "spd": 22,
        "spa": 53,
        "spdef": 45
      },
      "bst": 355,
      "statsSource": "Pokerex"
    },
    {
      "number": 305,
      "name": "Carracosta",
      "region": "",
      "location": "",
      "rarity": "",
      "notes": "",
      "availability": "Evolution / special",
      "sprite": "assets/sprites/carracosta.png",
      "types": [
        "water",
        "rock"
      ],
      "evolvesFrom": [
        304
      ],
      "evolvesTo": [],
      "stats": {
        "hp": 74,
        "atk": 108,
        "def": 133,
        "spd": 32,
        "spa": 83,
        "spdef": 65
      },
      "bst": 495,
      "statsSource": "Pokerex"
    },
    {
      "number": 306,
      "name": "Aerodactyl",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/aerodactyl.png",
      "types": [
        "rock",
        "flying"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 80,
        "atk": 105,
        "def": 65,
        "spd": 130,
        "spa": 60,
        "spdef": 75
      },
      "bst": 515,
      "statsSource": "Pokerex"
    },
    {
      "number": 307,
      "name": "Scream_Tail",
      "region": "",
      "location": "Ancient Mirroh",
      "rarity": "",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-985.png",
      "types": [
        "fairy",
        "psychic"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 115,
        "atk": 65,
        "def": 99,
        "spd": 111,
        "spa": 65,
        "spdef": 115
      },
      "bst": 570,
      "statsSource": "Pokerex"
    },
    {
      "number": 308,
      "name": "Flutter_Mane",
      "region": "",
      "location": "Vine Caverns",
      "rarity": "",
      "notes": "Scan the ancient carving with the Archaeolens 2.0",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-987.png",
      "types": [
        "ghost",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 55,
        "atk": 55,
        "def": 55,
        "spd": 135,
        "spa": 135,
        "spdef": 135
      },
      "bst": 570,
      "statsSource": "Pokerex"
    },
    {
      "number": 309,
      "name": "Great_Tusk",
      "region": "",
      "location": "Static Cave",
      "rarity": "",
      "notes": "Scan the ancient carving with the Archaeolens 2.0",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-984.png",
      "types": [
        "ground",
        "fighting"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 115,
        "atk": 131,
        "def": 131,
        "spd": 87,
        "spa": 53,
        "spdef": 53
      },
      "bst": 570,
      "statsSource": "Pokerex"
    },
    {
      "number": 310,
      "name": "Roaring_Moon",
      "region": "",
      "location": "Mirroh Interior B1F",
      "rarity": "",
      "notes": "Scan the ancient carving with the Archaeolens 2.0",
      "availability": "Available",
      "sprite": "assets/sprites/pokeapi-1005.png",
      "types": [
        "dragon",
        "dark"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 105,
        "atk": 139,
        "def": 71,
        "spd": 119,
        "spa": 55,
        "spdef": 101
      },
      "bst": 590,
      "statsSource": "Pokerex"
    },
    {
      "number": 311,
      "name": "Zarude",
      "region": "",
      "location": "Ivy Forest",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/zarude.png",
      "types": [
        "dark",
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 105,
        "atk": 120,
        "def": 105,
        "spd": 105,
        "spa": 70,
        "spdef": 95
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 312,
      "name": "Zeraora",
      "region": "",
      "location": "Route 6",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/zeraora.png",
      "types": [
        "electric"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 88,
        "atk": 112,
        "def": 75,
        "spd": 143,
        "spa": 102,
        "spdef": 80
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 313,
      "name": "Celebi",
      "region": "",
      "location": "Vilethorn Woods",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/celebi.png",
      "types": [
        "psychic",
        "grass"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 100,
        "atk": 100,
        "def": 100,
        "spd": 100,
        "spa": 100,
        "spdef": 100
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 314,
      "name": "Diancie",
      "region": "",
      "location": "Route 3 Caverns",
      "rarity": "Unique",
      "notes": "",
      "availability": "Available",
      "sprite": "assets/sprites/diancie.png",
      "types": [
        "rock",
        "fairy"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 50,
        "atk": 100,
        "def": 150,
        "spd": 50,
        "spa": 100,
        "spdef": 150
      },
      "bst": 600,
      "statsSource": "Pokerex"
    },
    {
      "number": 315,
      "name": "Koraidon",
      "region": "",
      "location": "UNOBTAINABLE",
      "rarity": "Unique",
      "notes": "Sorry! Prof Tenebris has it for research",
      "availability": "Unobtainable",
      "sprite": "assets/sprites/pokeapi-1007.png",
      "types": [
        "fighting",
        "dragon"
      ],
      "evolvesFrom": [],
      "evolvesTo": [],
      "stats": {
        "hp": 100,
        "atk": 135,
        "def": 115,
        "spa": 85,
        "spdef": 100,
        "spd": 135
      },
      "bst": 670,
      "statsSource": "Canonical fallback"
    }
  ],
  "megas": [
    {
      "number": 1,
      "name": "Medicham",
      "sprite": "assets/sprites/medicham-mega.png"
    },
    {
      "number": 2,
      "name": "Aggron",
      "sprite": "assets/sprites/aggron-mega.png"
    },
    {
      "number": 3,
      "name": "Altaria",
      "sprite": "assets/sprites/altaria-mega.png"
    },
    {
      "number": 4,
      "name": "Lopunny",
      "sprite": "assets/sprites/lopunny-mega.png"
    },
    {
      "number": 5,
      "name": "Banette",
      "sprite": "assets/sprites/banette-mega.png"
    },
    {
      "number": 6,
      "name": "Ampharos",
      "sprite": "assets/sprites/ampharos-mega.png"
    },
    {
      "number": 7,
      "name": "Gyarados",
      "sprite": "assets/sprites/gyarados-mega.png"
    },
    {
      "number": 8,
      "name": "Mawile",
      "sprite": "assets/sprites/mawile-mega.png"
    },
    {
      "number": 9,
      "name": "Gardevoir",
      "sprite": "assets/sprites/gardevoir-mega.png"
    },
    {
      "number": 10,
      "name": "Gallade",
      "sprite": "assets/sprites/gallade-mega.png"
    },
    {
      "number": 11,
      "name": "Houndoom",
      "sprite": "assets/sprites/houndoom-mega.png"
    },
    {
      "number": 12,
      "name": "Metagross",
      "sprite": "assets/sprites/metagross-mega.png"
    },
    {
      "number": 13,
      "name": "Scizor",
      "sprite": "assets/sprites/scizor-mega.png"
    },
    {
      "number": 14,
      "name": "Kangaskhan",
      "sprite": "assets/sprites/kangaskhan-mega.png"
    },
    {
      "number": 15,
      "name": "Pinsir",
      "sprite": "assets/sprites/pinsir-mega.png"
    },
    {
      "number": 16,
      "name": "Sharpedo",
      "sprite": "assets/sprites/sharpedo-mega.png"
    },
    {
      "number": 17,
      "name": "Manectric",
      "sprite": "assets/sprites/manectric-mega.png"
    },
    {
      "number": 18,
      "name": "Absol",
      "sprite": "assets/sprites/absol-mega.png"
    }
  ],
  "importantItems": [
    {
      "name": "Trolling Rod",
      "function": "Lets you fish for non-fish shaped Pokemon",
      "location": "Bottom of Route 6, just next to Pelluca City, there's a fisherman. Complete his quest."
    },
    {
      "name": "Diamond",
      "function": "Needed for Diancie",
      "location": "Complete the rich lady pearl quest in Galecrest and then talk to the husband"
    },
    {
      "name": "Ruby",
      "function": "Needed for Diancie",
      "location": "It's at the peak of Mt. Ceram. It's just lying there so you can get it any time after you gain access to backtracking."
    },
    {
      "name": "Sapphire",
      "function": "Needed for Diancie",
      "location": "It's deep inside Mt. Mirroh. Search the interior again after getting the Trekking Boots, and you'll find a spot to fall down deeper. There's an ice puzzle there - complete it and get the Sapphire."
    },
    {
      "name": "HM Strength",
      "function": "Lets you move boulders around",
      "location": "Beat Melea at the Somber HQ (inside the theater) in Silversun City. You'll get it from the detectives afterwards."
    }
  ],
  "sourceNotes": {
    "pokedex": "Hey everyone! Thanks for playing the game.\n\nWhile detailed documentation is in the works, please refer to this sheet here for the Pokemon locations. Special notes have been made for Pokemon that need it (eg special encounters, or Pokemon necessary for story/quest progression). Note that some Pokemon are unobtainable - those have been marked accordingly.\nThe primary location for each Pokemon is listed. A few of them may be available in a couple of other places. \nRarities: The lowest rarity in the game is 10-15% which is Rare. Common is 25-40%, Uncommon is ~15-25%\n\nIf you search for over 30-40 encounters and don't get what you want, don't bother! Report it instead. No Pokemon is less than 10% rate so you should definitely see at least one in 30-40 encounters.\n\nIf you notice any discrepancies, please fill the Bug Reports form! I'll get to it right away.\nAn item/TM/HM Dex is also on the way!",
    "megas": "Here's a list of the megas available.\n\nYou can only get 4 mega stones in the game:\n1. Aerodactylite\n2. Choice of one of the Kanto starter megas\n3. Choice of one of the rest\n4. Diancite\n\nSo the list below is all part of a single choice (you can only choose one of these megas below)",
    "importantItems": "Some important item locations are listed below.\nFor spoiler purposes, the text in all of these is WHITE. So if you want to see what's written, just click on the cell and you can see the text in the box above which shows the cell text.\n\nUpdating this will take some time, apologies!"
  }
};
